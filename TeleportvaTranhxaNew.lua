...
local AVOID_DISTANCE = 25 -- Tăng khoảng cách tránh xa
local lastTeleportTime = 0
local TELEPORT_COOLDOWN = 0.2

-- Biến chống stun
local lastAvoidTime = 0
local AVOID_COOLDOWN = 0.03 -- Cooldown cực ngắn
local isForceMoving = false
...
-- PHƯƠNG PHÁP DI CHUYỂN ĐẶC BIỆT ĐỂ BYPASS STUN
local function forceMoveToPosition(position)
    ...
    -- Phương pháp 1: Sử dụng BodyVelocity (ưu tiên cao nhất)
    local bodyVelocity = Instance.new("BodyVelocity")
    bodyVelocity.Velocity = (position - playerRoot.Position).Unit * 100
    bodyVelocity.MaxForce = Vector3.new(100000, 0, 100000) -- Chỉ tác động X và Z
    bodyVelocity.P = 10000
    bodyVelocity.Parent = playerRoot
    
    -- Phương pháp 2: Đồng thời sử dụng CFrame
    pcall(function()
        playerRoot.CFrame = CFrame.new(position) * (playerRoot.CFrame - playerRoot.Position)
        success = true
    end)
    
    -- Giữ BodyVelocity trong 0.2 giây để đảm bảo di chuyển
...
local function teleportClose(target)
    ...
    -- KHÔNG KIỂM TRA VẬT CẢN - DI CHUYỂN TRỰC TIẾP
    local newCFrame = CFrame.new(behindPosition) * (playerRoot.CFrame - playerRoot.Position)
    
    -- SỬ DỤNG PCALL VÀ THỬ NHIỀU LẦN
    local success = false
    for i = 1, 3 do
...
-- Tránh xa mục tiêu - BYPASS STUN HOÀN TOÀN
local function avoidTargetOnce()
    -- Kiểm tra cooldown cực ngắn
    local currentTime = tick()
    if currentTime - lastAvoidTime < AVOID_COOLDOWN or isForceMoving then
        return false
    end
    ...
    -- Vị trí mục tiêu sau khi tránh xa
    local targetPosition = playerRoot.Position + direction * AVOID_DISTANCE
    
    -- ĐÁNH DẤU ĐANG DI CHUYỂN
    isForceMoving = true
    
    -- THỰC HIỆN DI CHUYỂN BẰNG PHƯƠNG PHÁP ĐẶC BIỆT
    local success = forceMoveToPosition(targetPosition)
    ...
    if success then
        ...
        avoidButton.BackgroundColor3 = Color3.fromRGB(255, 100, 0)
        avoidButton.Text = "THẤT BẠI"
    end
...
-- Hàm xử lý click nút tránh xa (BYPASS STUN)
...
print("✅ Teleport & Avoid Script Đã Sẵn Sàng! (Bypass stun hoàn toàn - BodyVelocity + CFrame)")
