...
local SkillCategory = {
    M1          = { baseRadius = 15, maxRange = 20,  buffer = 0.3, minDodge = 0.6, height = 100 },
    Melee       = { baseRadius = 25, maxRange = 35,  buffer = 0.5, minDodge = 1.0, height = 100 },
    LungeDash   = { baseRadius = 45, maxRange = 75,  buffer = 0.8, minDodge = 1.2, height = 100 },
    BeamLine    = { baseRadius = 35, maxRange = 120, buffer = 1.2, minDodge = 1.5, height = 120, cone = 90 },
    WideAoE     = { baseRadius = 70, maxRange = 100, buffer = 1.5, minDodge = 1.8, height = 150 },
    UltimateAoE = { baseRadius = 120,maxRange = 160, buffer = 3.5, minDodge = 2.5, height = 350 },
}
...
-- Dynamic Threat & Directional Cone Calculation Algorithm
local function evaluateSkillThreat(rawId, enemyHrp, localHrp)
    local skill = SkillDatabase[rawId]
    if not skill then
        return true, 25, 0.5, 100, 1.0
    end

    local cat = skill.category
    local relPos = localHrp.Position - enemyHrp.Position
    local dist2D = math.sqrt(relPos.X * relPos.X + relPos.Z * relPos.Z)
    local deltaY = math.abs(relPos.Y)

    if deltaY > 60 and cat ~= SkillCategory.UltimateAoE then
        return false, cat.baseRadius, cat.buffer, cat.height, cat.minDodge
    end

    if cat.cone and cat.cone < 360 then
        local enemyLook = enemyHrp.CFrame.LookVector
        local dirToPlayer = Vector3.new(relPos.X, 0, relPos.Z).Unit
        local dot = enemyLook:Dot(dirToPlayer)
        local angle = math.deg(math.acos(math.clamp(dot, -1, 1)))
        if angle > (cat.cone / 2) then
            return false, cat.baseRadius, cat.buffer, cat.height, cat.minDodge
        end
    end

    local enemyVel = enemyHrp.AssemblyLinearVelocity
    local closingSpeed = -enemyVel:Dot(relPos.Unit)
    local velocityBuffer = math.max(0, closingSpeed * 0.25)
    
    local effectiveThreatRange = SAFE_ZONE_RADIUS + cat.baseRadius + velocityBuffer
    local isThreat = dist2D <= effectiveThreatRange or dist2D <= (cat.maxRange + velocityBuffer)
    ...
    return isThreat, cat.baseRadius, buffer, height, minDodge
end

-- HÀM HẠ CÁCH AN TOÀN TRIỆT TIÊU 100% HIỆU ỨNG NHẢY/BUMP/CHÌM ĐẤT
local function safeLandCharacter(heightOffset, savedCFrame)
    ...
    local estimatedY = savedCFrame and savedCFrame.Position.Y or (currentPos.Y - (heightOffset or activeDodgeHeight))
    ...
    local rayHeightOrigin = math.max(currentPos.Y + 10, estimatedY + 150)
    local rayDistance = math.max(rayHeightOrigin - (estimatedY - 150), 300)
    ...
    local finalY
    if highestGroundY then
        finalY = highestGroundY + legOffset
    elseif savedCFrame then
        finalY = savedCFrame.Position.Y
    else
        finalY = estimatedY
    end
...
-- Sky-High Beatdown Escape System (+5000 studs above FallenPartsDestroyHeight)
...
-- Direct Dual-Array Structural Ghost Generator
...
-- Dynamic Invisibility Engine 2.0
    ...
    if enable then
        ...
        RunService:BindToRenderStep("InvisCamAndGhostSync", 202, function()
            ...
            for i = 1, partCount do
                ...
                if rp and rp.Parent and gp and gp.Parent then
...
_G.LH_SetAntiHacker = function(enable)
        ...
        pcall(function()
            ...
            for _, obj in ipairs(myChar:GetDescendants()) do
                if obj:IsA("Weld") or obj:IsA("ManualWeld") or obj:IsA("WeldConstraint") or obj:IsA("RopeConstraint") or obj:IsA("Seat") then
                    if not obj:IsDescendantOf(script) then
                        obj:Destroy()
                    end
                end
            end
    ...
    antiHackerHeartbeatConn = RunService.Heartbeat:Connect(function(dt)
        pcall(function()
            ...
            if hrp.AssemblyLinearVelocity.Magnitude > 150 or hrp.AssemblyAngularVelocity.Magnitude > 100 then
            ...
            if currentPos.Y < (destroyHeight + 50) then
                hrp.AssemblyLinearVelocity = Vector3.zero
            ...
            if lastValidCFrame and not isDodging and not C.InvisibleOn then
                local maxAllowedDist = ((C.SpeedOn and C.Speed or 16) * dt) + 25
                local deltaDist = (currentPos - lastValidCFrame.Position).Magnitude
                if deltaDist > maxAllowedDist then
...
local function triggerDynamicDodge(enemyTrack, enemyChar)
    ...
    if skillInfo and skillInfo.category then
        activeDodgeHeight = skillInfo.category.height
    else
    ...
    RunService:BindToRenderStep("DodgeCamAndGhostSync", CAM_PRIORITY, function()
        if not isDodging or not dodgeLocalHrp or not dodgeLocalHrp.Parent then return end
    ...
    while currentTrack and not cancelDodgeSignal do
        ...
        local safetyBuffer = catInfo and catInfo.buffer or 0.5
        local animDuration = math.min(math.max(baseLength + safetyBuffer, 1.2), 8)
        local MIN_DODGE_DURATION = catInfo and catInfo.minDodge or 0.6
...
-- Single Central Heartbeat Loop for Ranged/Dash Tracking
RunService.Heartbeat:Connect(function()
    ...
    for i = #ActiveTrackingTasks, 1, -1 do
        ...
        if not track or not track.IsPlaying or (currentTime - taskData.StartTime > 3.5) or isDodging then
            table.remove(ActiveTrackingTasks, i)
        else
            local enemyHrp = getRootPart(ownerChar)
            if enemyHrp and enemyHrp.Parent then
                local isThreat, radius, buffer, height, minDodge = evaluateSkillThreat(taskData.RawId, enemyHrp, myHrp)
                if isThreat then
                    table.remove(ActiveTrackingTasks, i)
                    task.spawn(function()
...
local function onAnimationPlayed(track, animator, boundChar)
    ...
    local myChar = lp.Character
    local myHrp = getRootPart(myChar)
    local realOwnerChar = boundChar or getCharacterFromInstance(animator)
    if not realOwnerChar then return end

    -- Self attack cancellation
    ...
    if rawId == "11343318134" and C.SaitamaOn then
        ...
        if myHrp and enemyHrp and (myHrp.Position - enemyHrp.Position).Magnitude <= 25 then
    ...
    if skillData then
        ...
        if C.AntiHackerOn then
            ...
            elseif hero == "AllM1" then isEnabled = true
            end
        end
    end
    ...
    local isThreat, radius, buffer, height, minDodge = evaluateSkillThreat(rawId, enemyHrp, myHrp)
...
local function hookAnimator(animator)
    ...
    connections.AnimationPlayed = animator.AnimationPlayed:Connect(function(track)
        onAnimationPlayed(track, animator, boundChar or getCharacterFromInstance(animator))
    end)
