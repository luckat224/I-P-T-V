...
-- ⚡ ULTRA REAL-TIME CAMERA SUBJECT LOCK & GROUND HAZARD ENGINE ⚡
local activeCamFocusPart = nil

local function lockCameraToGhost(ghostModel, groundCFrame)
    if not camera then return end
    
    if activeCamFocusPart then 
        pcall(function() activeCamFocusPart:Destroy() end) 
        activeCamFocusPart = nil 
    end
    
    local camPart = Instance.new("Part")
    camPart.Name = "LH_CamFocusPart"
    ...
    camPart.CFrame = groundCFrame
    camPart.Parent = Workspace
    activeCamFocusPart = camPart

    pcall(function() camera.CameraSubject = camPart end)
end

local function restoreCameraSubject()
    if camera then
        if dodgeLocalHum and dodgeLocalHum.Health > 0 then
            pcall(function() camera.CameraSubject = dodgeLocalHum end)
...
local function handleSaitamaBeatdownEscape(enemyChar)
    ...
    lockCameraToGhost(nil, currentCFrame)
...
local function triggerDynamicDodge(enemyTrack, enemyChar)
    ...
    -- ⚡ KHÓA CAMERA SUBJECT VÀO GHOST/GROUND — TRIỆT TIÊU 100% HIỆU ỨNG DỰT CAMERA LÊN TRỜI
    lockCameraToGhost(activeGhostModel, dodgeSavedCFrame)

    pcall(function() RunService:UnbindFromRenderStep("DodgeCamAndGhostSync") end)
    -- Render Priority: Camera.Value - 1 (1999) chạy TRƯỚC Roblox Camera Script để triệt tiêu 100% 1-frame rendering desync
    local CAM_PRIORITY = Enum.RenderPriority.Camera.Value - 1
    ...
    RunService:BindToRenderStep("DodgeCamAndGhostSync", CAM_PRIORITY, function()
        ...
        -- Cập nhật CFrame cho Ground CamFocusPart nếu player di chuyển
        if activeCamFocusPart and activeCamFocusPart.Parent then
            activeCamFocusPart.CFrame = CFrame.new(currentPos.X, dodgeSavedCFrame.Position.Y, currentPos.Z) * dodgeLocalHrp.CFrame.Rotation
        end
        ...
        for i = 1, ghostPartCount do
            ...
            if rp and gp and rp.Parent and gp.Parent then
                gp.CFrame = rp.CFrame - Vector3.new(0, activeDodgeHeight, 0)
            end
        end
    end)
    ...
    -- Atomic Teardown: Land HRP first, restore CameraSubject, then destroy ghost objects
    if isDodging then
