...
local SaitamaSkills = {
    ["12447707844"] = true, -- Saitama Serious Punch / Ult
    ["11343318134"] = true, -- Beatdown
    ["11365563255"] = true, -- Table Flip
    ["12983333733"] = true, -- Omnidirectional Punches
    ["13927612951"] = true, -- Shove
    ["12510170988"] = true, -- Death Counter
    ["10471336737"] = true, -- Consecutive Punches
    ["10466974800"] = true, -- Normal Punch
    ["10468665991"] = true, -- Uppercut
    ["12447226503"] = true, -- Serious Table Flip Charge
    ["12447228807"] = true, -- Serious Table Flip Slam
    ["12983416823"] = true, -- Omnidirectional Punches Finisher
}
...
-- BẢNG ANIMATION ID DÀNH RIÊNG CHO DEATH COUNTER SAITAMA
local SaitamaDeathCounterAnims = {
    ["12510170988"] = true, -- Saitama Death Counter
    ["12447707844"] = true, -- Saitama Serious Counter / Ult
    ["11343318134"] = true, -- Saitama Beatdown / Death Counter
}
...
-- HÀM HẠ CÁNH AN TOÀN 100% TRIỆT TIÊU HIỆU ỨNG NHẢY LÊN TRỜI / GIẬT XÁC / CHÌM ĐẤT
local function safeLandCharacter(heightOffset, savedCFrame)
    ...
    if fakePlatform then pcall(function() fakePlatform.CanCollide = false end) end
    if invisFakePlatform then pcall(function() invisFakePlatform.CanCollide = false end) end

    local currentPos = dodgeLocalHrp.Position
    local targetX = savedCFrame and savedCFrame.Position.X or currentPos.X
    local targetZ = savedCFrame and savedCFrame.Position.Z or currentPos.Z
    local estimatedY = savedCFrame and savedCFrame.Position.Y or (currentPos.Y - heightOffset)

    local raycastParams = RaycastParams.new()
    raycastParams.FilterType = RaycastFilterType.Exclude
    local filterList = {dodgeLocalChar}
    if fakePlatform then table.insert(filterList, fakePlatform) end
    if invisFakePlatform then table.insert(filterList, invisFakePlatform) end
    ...
    local rayOrigin = Vector3.new(targetX, estimatedY + 25, targetZ)
    ...
    local finalPosition
    if rayResult then
        finalPosition = Vector3.new(targetX, rayResult.Position.Y + legOffset + 0.05, targetZ)
    elseif savedCFrame then
        finalPosition = savedCFrame.Position
    else
        finalPosition = Vector3.new(targetX, estimatedY, targetZ)
    end

    local finalCFrame = CFrame.new(finalPosition) * (savedCFrame and savedCFrame.Rotation or dodgeLocalHrp.CFrame.Rotation)
...
local function handleSaitamaBeatdownEscape()
    ...
    if dodgeLocalHrp and dodgeLocalHrp.Parent then
        safeLandCharacter(BEATDOWN_ESCAPE_DEPTH, currentCFrame)
    end
...
_G.LH_SetInvisible = function(enable)
    if enable then
        ...
        invisFakePlatform.Size = Vector3.new(2000, 10, 2000)
        invisFakePlatform.Position = invisSavedCFrame.Position + Vector3.new(0, INVIS_HEIGHT - legOffset - 5, 0)
        ...
        RunService:BindToRenderStep("InvisCamAndGhostSync", 202, function()
            -- KHOÁ CỨNG VỊ TRÍ XÁC THẬT TRÊN TRỜI MỖI FRAME TÀNG HÌNH (TRÁNH BỊ RỚT MAP)
            if dodgeLocalHrp and dodgeLocalHrp.Parent and invisSavedCFrame then
                local targetInvisAirPos = invisSavedCFrame.Position + Vector3.new(0, INVIS_HEIGHT, 0)
                dodgeLocalHrp.CFrame = CFrame.new(targetInvisAirPos) * (dodgeLocalHrp.CFrame.Rotation)
                dodgeLocalHrp.AssemblyLinearVelocity = Vector3.zero
                dodgeLocalHrp.AssemblyAngularVelocity = Vector3.zero
            end
        ...
        safeLandCharacter(INVIS_HEIGHT, invisSavedCFrame)
...
local function emergencyDodgeCleanup(skipLand)
    ...
    if not skipLand and isDodging and dodgeSavedCFrame and dodgeLocalHrp and dodgeLocalHum and dodgeLocalHum.Health > 0 then
        safeLandCharacter(activeTeleportHeight or TELEPORT_HEIGHT, dodgeSavedCFrame)
    end
...
lp.CharacterAdded:Connect(function()
    -- RESET DODGE STATE KHI RESPAWN (BỎ QUA LAND ĐỂ TRÁNH BỊ DỰT VỀ VỊ TRÍ CHẾT CŨ)
...
local function getAttackRadius(rawId)
    if AoESkills[rawId] or SaitamaSkills[rawId] then return 120 end -- Siêu bán kính 120 studs cho Saitama & AoE Ult
...
local function getSkillSafetyBuffer(rawId)
    if AoESkills[rawId] or SaitamaSkills[rawId] then return 3.5 end -- AoE / Saitama Ult: Nổ rộng kéo dài, thêm 3.5s
    if TatsumakiSkills[rawId] then return 1.5 end   -- Lốc xoáy xoay lâu
    if CyborgSkills[rawId] then return 1.2 end       -- Tên lửa/lửa cháy lan
...
local function triggerDynamicDodge(enemyTrack, enemyChar)
    ...
    -- ĐỘ CAO NÉ ĐỘNG: CHIÊU ULTI SAITAMA / AOE BAY LÊN +350 STUDS GIÚP TRÁNH 100% CÁC CỤM ĐÁ NỔ
    local enemyRawId = enemyTrack and enemyTrack.Animation and string.match(enemyTrack.Animation.AnimationId, "%d+")
    local activeTeleportHeight = TELEPORT_HEIGHT
    if enemyRawId and (AoESkills[enemyRawId] or SaitamaSkills[enemyRawId]) then
        activeTeleportHeight = 350
    end

    -- WATCHDOG: Tự động reset sau 8 giây nếu kẹt (tránh đơ vĩnh viễn)
    ...
    -- LƯU VỊ TRÍ MẶT ĐẤT BAN ĐẦU CHUẨN XÁC
    dodgeSavedCFrame = dodgeLocalHrp.CFrame

    -- TELEPORT XÁC THẬT LÊN TRỜI TỨC THÌ 0MS TẠI ĐỘ CAO AN TOÀN
    dodgeLocalHrp.CFrame = dodgeSavedCFrame + Vector3.new(0, activeTeleportHeight, 0)
    dodgeLocalHrp.AssemblyLinearVelocity = Vector3.zero
    dodgeLocalHrp.AssemblyAngularVelocity = Vector3.zero

    -- TÍNH TOÁN KHOẢNG CÁCH CHÂN CHẠM ĐẤT CHUẨN XÁC 100%
    local legOffset = 3.0
    if dodgeLocalHum then
        if dodgeLocalHum.RigType == Enum.HumanoidRigType.R15 then
            legOffset = dodgeLocalHum.HipHeight + (dodgeLocalHrp.Size.Y / 2)
    ...
    fakePlatform.Size = Vector3.new(2000, 10, 2000)
    fakePlatform.Position = dodgeSavedCFrame.Position + Vector3.new(0, activeTeleportHeight - legOffset - 5, 0)
    ...
    local ghostOk, ghostModel, partMap = true, nil, {}
    ghostOk, ghostModel, partMap = pcall(createGhostClone, dodgeLocalChar)
    if not ghostOk then ghostModel = nil; partMap = {} end
    activeGhostModel = ghostModel

    pcall(function() RunService:UnbindFromRenderStep("DodgeCamAndGhostSync") end)

    RunService:BindToRenderStep("DodgeCamAndGhostSync", 201, function()
        camera.CFrame = camera.CFrame - Vector3.new(0, activeTeleportHeight, 0)
        
        -- KHOÁ CỨNG VỊ TRÍ XÁC THẬT TRÊN TRỜI MỖI FRAME (TRÁNH GHOST BỊ BẮN/RỚT XUỐNG VOID MAP)
        if dodgeLocalHrp and dodgeLocalHrp.Parent and dodgeSavedCFrame then
            local targetAirPos = dodgeSavedCFrame.Position + Vector3.new(0, activeTeleportHeight, 0)
            dodgeLocalHrp.CFrame = CFrame.new(targetAirPos) * (dodgeLocalHrp.CFrame.Rotation)
            dodgeLocalHrp.AssemblyLinearVelocity = Vector3.zero
            dodgeLocalHrp.AssemblyAngularVelocity = Vector3.zero
        end
        ...
        if partMap then
            for realPart, ghostPart in pairs(partMap) do
                if realPart and realPart.Parent and ghostPart and ghostPart.Parent then
                    ghostPart.CFrame = realPart.CFrame - Vector3.new(0, activeTeleportHeight, 0)
                end
            end
        end
    end)

    -- VÒNG LẶP CHỜ NÉ (HỖ TRỢ NÉ CHUỖI COMBO LIÊN TỤC KHÔNG TÁCH ĐÔI / KHÔNG BAY TRỜI)
    ...
    while currentTrack and not cancelDodgeSignal do
        ...
        local baseLength = (currentTrack and currentTrack.Length and currentTrack.Length > 0) and currentTrack.Length or 2.0
        local safetyBuffer = getSkillSafetyBuffer(rawAnimId)
        local animDuration = math.max(baseLength + safetyBuffer, 1.5)
        animDuration = math.min(animDuration, 8)
        
        -- RIÊNG CÁC CHIÊU ULTI SAITAMA & AOE: NÉ TỐI THIỂU 2.5 GIÂY ĐỂ TRÁNH BỊ HẠ CÁCH SỚM KHI VẠT NỔ CÒN ĐANG CHẠY
        local MIN_DODGE_DURATION = 1.0
        if rawAnimId and (AoESkills[rawAnimId] or SaitamaSkills[rawAnimId]) then
            MIN_DODGE_DURATION = 2.5
        end

        local startTime = os.clock()
        while not cancelDodgeSignal and (os.clock() - startTime < animDuration + 0.05) do
            ...
            if currentTrack and not currentTrack.IsPlaying and (os.clock() - startTime >= MIN_DODGE_DURATION) then break end
            RunService.RenderStepped:Wait()
        end
        ...
        -- KIỂM TRA HÀNG ĐỢI NÉ TIẾP THEO IN COMBO
        local nextTrack = nil
    ...
    -- HẠ CÁCH AN TOÀN VỀ MẶT ĐẤT (0.000% JUMP / BOUNCE / GROUND PENETRATION)
    safeLandCharacter(activeTeleportHeight, dodgeSavedCFrame)
...
-- HÀM TRUY TÌM CHARACTER THẬT SỰ (CHỨA HUMANOIDROOTPART)
local function getActualCharacter(inst)
    if not inst then return nil end
    local current = inst
    while current and current ~= workspace do
        if current:IsA("Model") and current:FindFirstChild("HumanoidRootPart") then
            return current
        end
...
local function hookAnimator(animator, ownerChar)
    ...
    local function onAnimationPlayed(track)
        ...
        -- Dynamic resolution of local player character and parts
        local myChar = lp.Character
        local myHrp = myChar and myChar:FindFirstChild("HumanoidRootPart")

        -- CHỈ HỦY NÉ KHI BẠN CHỦ ĐỘNG BẤM SKILL/ĐẤM M1 REAL
        local realOwnerChar = getActualCharacter(ownerChar) or getActualCharacter(animator)
        if realOwnerChar and myChar and (realOwnerChar == myChar or realOwnerChar:IsDescendantOf(myChar)) then
            if isDodging then
                local p = track.Priority
                local isExplicitAttack = (p == Enum.AnimationPriority.Action3) or 
                                         (p == Enum.AnimationPriority.Action4) or
                                         (rawId and (AllM1Skills[rawId] or GarouV1Skills[rawId] or SaitamaSkills[rawId] or 
        ...
        if rawId and SaitamaDeathCounterAnims[rawId] then
            if realOwnerChar ~= myChar and C.DeathCounterOn then
                showDeathCounterWarning(realOwnerChar, track)
            end
        end

        if rawId == "11343318134" and C.SaitamaOn then
            local isLocalVictim = (realOwnerChar == myChar) or (myChar and realOwnerChar:IsDescendantOf(myChar))
            if not isLocalVictim and myHrp and realOwnerChar:FindFirstChild("HumanoidRootPart") then
                local dist = (myHrp.Position - realOwnerChar.HumanoidRootPart.Position).Magnitude
                if dist <= 25 then isLocalVictim = true end
            end
        ...
        -- ====================================================================
        -- ⚡ HỆ THỐNG AUTO DODGE SIÊU CẤP V4 (UNIVERSAL 360° SAFE ZONE 100% ACCURACY) ⚡
        -- ====================================================================
        local isEnabled = false
        local isM1 = rawId and AllM1Skills[rawId]

        if C.AntiHackerOn then
            if isM1 or GarouV1Skills[rawId] or SaitamaSkills[rawId] or GarouV2Skills[rawId] or CyborgSkills[rawId] or NinjaSkills[rawId] or TrashCanSkills[rawId] or MetalBatSkills[rawId] or TatsumakiSkills[rawId] or SamuraiSkills[rawId] or ChildEmperorSkills[rawId] or ZombiemanSkills[rawId] or SuiryuSkills[rawId] then
                isEnabled = true
            end
        else
            if C.GarouV1On and (GarouV1Skills[rawId] or isM1) then isEnabled = true
            elseif C.SaitamaOn and (SaitamaSkills[rawId] or isM1) then isEnabled = true
            elseif C.GarouV2On and (GarouV2Skills[rawId] or isM1) then isEnabled = true
            elseif C.CyborgOn and (CyborgSkills[rawId] or isM1) then isEnabled = true
            elseif C.NinjaOn and (NinjaSkills[rawId] or isM1) then isEnabled = true
            elseif C.TrashCanOn and (TrashCanSkills[rawId] or isM1) then isEnabled = true
            elseif C.MetalBatOn and (MetalBatSkills[rawId] or isM1) then isEnabled = true
            elseif C.TatsumakiOn and (TatsumakiSkills[rawId] or isM1) then isEnabled = true
            elseif C.SamuraiOn and (SamuraiSkills[rawId] or isM1) then isEnabled = true
            elseif C.ChildEmperorOn and (ChildEmperorSkills[rawId] or isM1) then isEnabled = true
            elseif C.ZombiemanOn and (ZombiemanSkills[rawId] or isM1) then isEnabled = true
            elseif C.SuiryuOn and (SuiryuSkills[rawId] or isM1) then isEnabled = true
            end
        end
        ...
        if isEnabled then
            ...
            local enemyHrp = realOwnerChar:FindFirstChild("HumanoidRootPart") or realOwnerChar.PrimaryPart
            if not enemyHrp then return end

            local attackRadius = getAttackRadius(rawId)
            local maxThreatRange = SAFE_ZONE_RADIUS + attackRadius

            -- KHOẢNG CÁCH 2D MẶT PHẲNG NẰM NGANG (BAO PHỦ 360 ĐỘ 100% CHÍNH XÁC)
            ...
            -- NÉ 360 ĐỘ TỨC THÌ 0MS
            if dist2D <= maxThreatRange then
                task.spawn(function()
                    triggerDynamicDodge(track, realOwnerChar)
                end)
            else
                -- LUỒNG TRUY BÁM THỜI GIAN THỰC CHO SKILL TẦM XA LƯỚT/BAY TỚI
                task.spawn(function()
                    local startTime = os.clock()
...
-- HOOK TOÀN BỘ NHÂN VẬT (WORKSPACE, WORKSPACE.LIVE, PLAYERS)
local function scanAndHook(container)
    if not container then return end
    for _, obj in pairs(container:GetDescendants()) do
        if obj:IsA("Animator") or obj:IsA("AnimationController") then
            local char = getActualCharacter(obj) or obj:FindFirstAncestorOfClass("Model")
