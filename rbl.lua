...
-- ===========================================================================
-- ⚡ ĐỘNG CƠ AUTO DODGE SYSTEM (GAROU V1 & SAITAMA & SMART AIM RADAR) ⚡
-- ===========================================================================
local GarouV1Skills = {
    ["12272894215"] = 0.72,
    ["12307656616"] = 0.79,
    ["12296882427"] = 0.68,
    ["13603396939"] = 3.01,
    ["13630786846"] = { time = 1.57, range = 65 }, -- Chiêu lướt xa (64.8 studs)
    ["14057231976"] = 1.90,
    ["12463072679"] = 0.89,
    ["12460977270"] = 1.97,
    ["12342141464"] = 5.44,
...
local SaitamaSkills = {
    ["12447707844"] = { time = 1.79, isUlt = true }, -- Ulti Saitama (Alert bên phải màn hình Mobile)
    ["11343318134"] = { time = 9.05, isBeatdown = true }, -- Hit Beatdown (Teleport 1000 studs trong 2s)
    ["11365563255"] = 8.34,
    ["12983333733"] = 9.13,
    ["13927612951"] = 2.50,
    ["12510170988"] = 1.19,
...
local SAFE_DISTANCE = 28
local CLOSE_AOE_DISTANCE = 12
local MIN_AIM_DOT = 0.3
...
-- Thông báo Mở Ulti Saitama (Tự nhiên + Avatar Headshot) bên phải màn hình Mobile
local function showUltNotification(playerObj)
    pcall(function()
        ...
        local pName = playerObj and playerObj.DisplayName or "Người Chơi"
        local userId = playerObj and playerObj.UserId or 1
        ...
        -- Ảnh Avatar Headshot của người vừa mở Ulti
        ...
        -- Hiệu ứng trượt nhẹ nhàng từ bên phải vào
        frame:TweenPosition(UDim2.new(1, -250, 0.35, 0), Enum.EasingDirection.Out, Enum.EasingStyle.Quad, 0.3, true)
...
-- Xử lý Teleport 100.000 studs trong 2s khi BẠN trúng chiêu Beatdown Saitama (ID 11343318134)
...
local function handleSaitamaBeatdownEscape()
    ...
    local BEATDOWN_ESCAPE_HEIGHT = 100000 -- ĐÃ ĐỔI THÀNH 100.000 STUDS

    print("⚠️ [11343318134] ĐÃ BỊ TRÚNG BEATDOWN SAITAMA -> TELEPORT 100.000 STUDS CHỜ 2S!")

    local currentCFrame = dodgeLocalHrp.CFrame

    local platform = Instance.new("Part")
    platform.Size = Vector3.new(500, 5, 500)
    platform.Position = currentCFrame.Position + Vector3.new(0, BEATDOWN_ESCAPE_HEIGHT - 3.5, 0)
...
local function triggerDodge(duration)
    ...
    local targetDuration = math.max(0.05, duration - EARLY_RETURN)
    local startTime = os.clock()
    
    while (os.clock() - startTime) < targetDuration do
        RunService.RenderStepped:Wait()
    end

    if dodgeLocalChar and dodgeLocalHrp and dodgeLocalHum.Health > 0 then
        dodgeLocalHrp.CFrame = dodgeLocalHrp.CFrame - Vector3.new(0, TELEPORT_HEIGHT, 0)
...
local function hookAnimator(animator, ownerChar)
    ...
    animator.AnimationPlayed:Connect(function(track)
        ...
        if rawId == "12447707844" and C.SaitamaOn then
            local pName = ownerChar.Name
            local pObj = Players:GetPlayerFromCharacter(ownerChar)
            if pObj then pName = pObj.DisplayName end
            showUltNotification(pName)
        end

        -- 3. XỬ LÝ AUTO DODGE CHO GAROU V1 VÀ SAITAMA
        local skillData = nil
        if C.GarouV1On and GarouV1Skills[rawId] then
            skillData = GarouV1Skills[rawId]
        elseif C.SaitamaOn and SaitamaSkills[rawId] then
            skillData = SaitamaSkills[rawId]
        end

        if skillData then
            if ownerChar == dodgeLocalChar or (dodgeLocalChar and ownerChar:IsDescendantOf(dodgeLocalChar)) then return end
            ...
            local duration = (type(skillData) == "table") and skillData.time or skillData
            local maxRange = (type(skillData) == "table" and skillData.range) and skillData.range or SAFE_DISTANCE
            ...
            if distSq <= maxRangeSq then
                ...
                if dist <= CLOSE_AOE_DISTANCE then
                    triggerDodge(duration)
                else
