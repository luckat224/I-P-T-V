...
local SkillCategory = {
    ...
    LungeDash   = { bodyHitRange = 16, trackRange = 80,  buffer = 0.8, minDodge = 1.2, height = 100 },
...
RunService.Heartbeat:Connect(function()
    ...
    for i = #ActiveTrackingTasks, 1, -1 do
        ...
        if not ownerChar or not ownerChar.Parent or (currentTime - taskData.StartTime > 4.0) then
            table.remove(ActiveTrackingTasks, i)
        else
            local enemyHrp = getRootPart(ownerChar)
            if enemyHrp and enemyHrp.Parent then
                local isBodyHit = evaluateSkillThreat(taskData.RawId, enemyHrp, myHrp)
                if isBodyHit then
                    table.remove(ActiveTrackingTasks, i)
                    task.spawn(function()
