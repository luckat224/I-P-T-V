...
local function triggerDynamicDodge(enemyTrack, enemyChar)
    ...
    -- WATCHDOG SAFETY: Tự động reset sau 6 giây nếu gặp lỗi bất ngờ (Tránh đơ vĩnh viễn)
    local currentDodgeSession = os.clock()
    task.delay(6.5, function()
        if isDodging and fakePlatform then
            emergencyDodgeCleanup()
        end
    end)
    ...
    local ok, err = pcall(function()
        ...
        -- TELEPORT XÁC THẬT LÊN TRỜI TỨC THÌ 0MS
        ...
        -- ĐẢM BẢO GIẢI PHÓNG BIND CŨ TRƯỚC KHI BIND MỚI (TRÁNH CẮT LUỒNG VÌ DUPLICATE BIND)
        pcall(function() RunService:UnbindFromRenderStep("DodgeCamAndGhostSync") end)
        ...
        -- ========================================================================
        -- 🧠 TÍNH THỜI GIAN NÉ THÔNG MINH
        -- ========================================================================
        local rawAnimId = enemyTrack and enemyTrack.Animation and string.match(enemyTrack.Animation.AnimationId, "%d+")

        local baseLength = (enemyTrack and enemyTrack.Length and enemyTrack.Length > 0) and enemyTrack.Length or 2.0
        local safetyBuffer = getSkillSafetyBuffer(rawAnimId)
        local animDuration = math.max(baseLength + safetyBuffer, 0.8)
        ...
        -- ========================================================================
        -- 🛡️ POST-DODGE THREAT SCAN: CHUYỂN HÀNG ĐỢI NÉ LIỀN MẠCH
        -- ========================================================================
        ...
        -- Nếu có skill/M1 tiếp theo đang active → Né liền mạch không hạ cánh
