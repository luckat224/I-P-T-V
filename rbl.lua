...
local function evaluateSkillThreat(rawId, enemyHrp, localHrp)
    ...
    -- Ground position reference: use saved ground CFrame if player is currently in elevated dodge
    ...
    -- Dynamic vertical threat threshold based on skill category/height
    local maxVerticalThreshold = (cat == SkillCategory.UltimateAoE) and 300 or math.max(60, height * 0.7)
    if deltaY > maxVerticalThreshold then
        return false, cat.baseRadius, buffer, height, minDodge
    end

    -- Directional cone check (exempt point-blank proximity < 12 studs for 360-degree sweep coverage)
    if cat.cone and cat.cone < 360 and dist2D > 12 then
        local flatRel = Vector3.new(relPos.X, 0, relPos.Z)
        if flatRel.Magnitude > 0.001 then
            local enemyLook = enemyHrp.CFrame.LookVector
            local dirToPlayer = flatRel.Unit
            local dot = enemyLook:Dot(dirToPlayer)
            local cosHalfCone = math.cos(math.rad(cat.cone / 2))
            if dot < cosHalfCone then
                return false, cat.baseRadius, buffer, height, minDodge
            end
        end
    end

    -- Accurate closing speed calculation (enemy approaching player)
    local relPosMag = relPos.Magnitude
    local dir3D = relPosMag > 0.001 and (relPos / relPosMag) or Vector3.zero
    local enemyVel = enemyHrp.AssemblyLinearVelocity
    local playerVel = localHrp.AssemblyLinearVelocity
    local relativeVel = enemyVel - playerVel
    local closingSpeed = relativeVel:Dot(dir3D)
    local velocityBuffer = math.max(0, closingSpeed * 0.35)
...
local function createGhostClone(char)
    ...
    for i = 1, #cloneDescendants do
        ...
        if obj:IsA("Script") or obj:IsA("LocalScript") or obj:IsA("Animator") 
           ...
           or obj:IsA("JointInstance") or obj:IsA("Attachment")
           or obj:IsA("Constraint") or obj:IsA("WeldConstraint")
...
_G.LH_SetAntiHacker = function(enable)
    ...
    antiHackerHeartbeatConn = RunService.Heartbeat:Connect(function(dt)
        pcall(function()
            ...
            local isCustomMoving = isDodging or C.InvisibleOn or C.FlyOn or C.NoclipOn or C.Tweening or C.Teleporting
...
RunService.Heartbeat:Connect(function()
    ...
    for i = #ActiveTrackingTasks, 1, -1 do
        ...
        if not track or not track.IsPlaying or (currentTime - taskData.StartTime > 3.5) then
