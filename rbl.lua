...
local function hookAnimator(animator, ownerChar)
    ...
    animator.AnimationPlayed:Connect(function(track)
        ...
        if isEnabled then
            ...
            -- THUẬT TOÁN BỌC HITBOX TỰ ĐỘNG KHÔNG GIAN CHO SKILL / ĐẠN / LỐC XOÁY / TÊN LỬA
            task.spawn(function()
                local startTime = os.clock()
                while track and track.IsPlaying and (os.clock() - startTime < 3.5) and not isDodging do
                    ...
                    -- 1. CHIÊU DIỆN RỘNG (AoE): Nổ bão bọc vòng tròn 80 studs
                    if AoESkills[rawId] then
                        if dist <= 80 then
                            shouldDodge = true
                        end
                    else
                        -- 2. BỌC HITBOX XUNG QUANH THÂN VÀ SKILL / TÊN LỬA / LỐC XOÁY (PROJECTILE / EFFECT MODELS)
                        if dist <= 10.5 then
                            shouldDodge = true
                        else
                            ...
                            -- Bọc vùng đạn/chiêu hướng tới bạn
                            if dot >= 0.25 and dist <= 14 then
                                shouldDodge = true
                            else
                                -- Kiểm tra nếu có vật thể tên lửa / lốc xoáy / kỹ năng văng ra từ đối thủ tiến sát bạn
                                for _, child in ipairs(workspace:GetChildren()) do
                                    if child ~= dodgeLocalChar and child ~= ownerChar then
                                        if child:IsA("BasePart") or child:IsA("Model") then
                                            local partPos = child:IsA("BasePart") and child.Position or (child.PrimaryPart and child.PrimaryPart.Position)
                                            if partPos then
                                                local pDist = (myPos - partPos).Magnitude
                                                local pFromEnemy = (partPos - enemyPos).Magnitude
                                                -- Vật thể skill nằm gần đối thủ/đường đạn và sắp bay chạm người bạn (<= 10.5 studs)
                                                if pDist <= 10.5 and pFromEnemy <= 120 then
                                                    shouldDodge = true
                                                    break
                                                end
                                            end
