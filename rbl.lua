...
local function isHeroToggleEnabled(heroName)
    ...
    elseif heroName == "GenericHero" then return true
    end
    return false
end
...
local function triggerDynamicDodge(enemyTrack, enemyChar)
    ...
    local catInfo = skillInfo and skillInfo.category
    local isPersistentSkill = (catInfo and catInfo.isPersistent) or (skillInfo and (skillInfo.isUlt or skillInfo.isPersistent))

    if skillInfo then
        activeDodgeHeight = skillInfo.height or (catInfo and catInfo.height) or TELEPORT_HEIGHT
    else
    ...
    while currentTrack and not cancelDodgeSignal do
        ...
        local currentSkill = rawAnimId and SkillDatabase[rawAnimId]
        local cat = currentSkill and currentSkill.category
        local isPersistent = isPersistentSkill or (cat and cat.isPersistent) or (currentSkill and (currentSkill.isUlt or currentSkill.isPersistent))

        local maxDodgeLimit = isPersistent and 4.0 or 2.5
        local MIN_DODGE_DURATION = isPersistent and (cat and cat.minDodge or 1.8) or 0.15
        local safeZoneRange = (cat and cat.bodyHitRange or 12) * 1.8

        local startTime = os.clock()
        while not cancelDodgeSignal and (os.clock() - startTime < maxDodgeLimit) do
            ...
            -- ⚡ THỜI GIAN THỰC CHO CHIÊU THƯỜNG / M1: Thoát NÉ NGAY LẬP TỨC khi animation kết thúc hoặc qua hit frame (>=75%)
            if not isPersistent then
                if trackEnded or (currentTrack and not currentTrack.IsPlaying) or (currentTrack and currentTrack.Length > 0 and currentTrack.TimePosition >= currentTrack.Length * 0.75) then
                    break
                end
            end

            -- ⚡ CHECK VÙNG AN TOÀN KHI DI CHUYỂN
            if dodgeSavedCFrame and dodgeLocalHrp and (elapsed >= MIN_DODGE_DURATION) then
                ...
                -- ⚡ GROUND HAZARD CHECK CHO CHIÊU DIỆN RỘNG (Table Flip / Ult AoE): Kiểm tra mặt đất SẠCH BÓNG mới hạ cánh
                if isPersistent then
                    if isGroundLocationSafe(dodgeSavedCFrame, 30) then
                        break
                    end
                end
            end
        ...
        if currentTrack then
            local nextRawId = currentTrack.Animation and string.match(currentTrack.Animation.AnimationId, "%d+")
            local nextInfo = nextRawId and SkillDatabase[nextRawId]
            if nextInfo then
                activeDodgeHeight = nextInfo.height or (nextInfo.category and nextInfo.category.height) or TELEPORT_HEIGHT
            end
        end
    end
