...
-- Thông báo Mở Ulti Saitama bên phải màn hình Mobile
local function showUltNotification(playerName)
    pcall(function()
        ...
        label.Position = UDim2.new(1, 10, 0.35, 0)
        label.BackgroundColor3 = Color3.fromRGB(25, 20, 30)
        label.Text = string.format("🔥 <b>%s</b>\nĐÃ MỞ ULTI SAITAMA!", playerName)
        label.RichText = true
        ...
        -- Hiệu ứng trượt từ lề phải vào
        label:TweenPosition(UDim2.new(1, -230, 0.35, 0), Enum.EasingDirection.Out, Enum.EasingStyle.Quad, 0.3, true)

        task.delay(4.5, function()
            if label and label.Parent then
                label:TweenPosition(UDim2.new(1, 10, 0.35, 0), Enum.EasingDirection.In, Enum.EasingStyle.Quad, 0.3, true, function()
                    label:Destroy()
                end)
            end
        end)
...
-- Xử lý Teleport 1000 studs trong 2s khi BẠN trúng chiêu Beatdown Saitama (ID 11343318134)
...
local function handleSaitamaBeatdownEscape()
    ...
    print("⚠️ [11343318134] ĐÃ BỊ TRÚNG BEATDOWN SAITAMA -> TELEPORT 1000 STUDS CHỜ 2S!")

    local currentCFrame = dodgeLocalHrp.CFrame

    local platform = Instance.new("Part")
    platform.Size = Vector3.new(500, 5, 500)
    platform.Position = currentCFrame.Position + Vector3.new(0, 1000 - 3.5, 0)
    platform.Anchored = true
    platform.Transparency = 1
    platform.Parent = workspace

    dodgeLocalHrp.CFrame = currentCFrame + Vector3.new(0, 1000, 0)
