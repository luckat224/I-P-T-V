...
addToggle(T1, "Tàng Hình (Invisible 500 Studs)", C.InvisibleOn, function(v)
    C.InvisibleOn = v
...
local GarouV2Skills = {
    ["109617620932970"] = 1.18,
    ["125955606488863"] = 1.06,
    ["72533960079559"] = 0.47,
    ["102989537449083"] = 0.30,
    ["131820095363270"] = 2.30,
    ["85025226664507"] = 2.10,
    ["71317401437256"] = 1.71,
    ["136465810903839"] = 3.83,
    ["107484339495811"] = 3.16,
    ["79527508933159"] = 1.54,
    ["139070970861356"] = 5.13,
}
...
local TrashCanSkills = {
    ["13813955149"] = 1.0, -- Ném thùng rác (trừ hao thời gian đạn bay)
}

local TELEPORT_HEIGHT = 100
local SAFE_ZONE_RADIUS = 45 -- VÙNG AN TOÀN SÁT THÂN (Bất kỳ skill ID nào xuất hiện/xâm nhập vào đây -> NÉ NGAY 100%)
local SAFE_ZONE_SQ = SAFE_ZONE_RADIUS * SAFE_ZONE_RADIUS
local SAFE_DISTANCE = 300 -- BÁN KÍNH QUÉT TỪ XA TOÀN MAP (300 studs): Bắt mọi skill/đạn tung ra từ đằng xa hướng về Vùng An Toàn
local MIN_AIM_DOT = -0.2 -- GÓC QUÉT DỰ ĐOÁN QUỸ ĐẠO (125 độ): Chỉ cần đạn/skill nhắm hướng về phía Vùng An Toàn là NÉ TỪ ĐẰNG XA
...
local function hookAnimator(animator, ownerChar)
    ...
    animator.AnimationPlayed:Connect(function(track)
        ...
        -- 3. XỬ LÝ AUTO DODGE TỰ ĐỘNG KHI SKILL GẦN TRÚNG BẠN (TẤT CẢ TƯỚNG)
        local duration = nil
        if C.GarouV1On and GarouV1Skills[rawId] then
            duration = GarouV1Skills[rawId]
        elseif C.SaitamaOn and SaitamaSkills[rawId] then
            duration = SaitamaSkills[rawId]
        elseif C.GarouV2On and GarouV2Skills[rawId] then
            duration = GarouV2Skills[rawId]
        elseif C.CyborgOn and CyborgSkills[rawId] then
            duration = CyborgSkills[rawId]
        elseif C.NinjaOn and NinjaSkills[rawId] then
            duration = NinjaSkills[rawId]
        elseif C.TrashCanOn and TrashCanSkills[rawId] then
            duration = TrashCanSkills[rawId]
        end
        ...
        if duration then
            ...
            -- 1. XÂM NHẬP VÙNG AN TOÀN SÁT THÂN (Dưới 45 studs) -> NÉ TỨC THÌ 100%!
            if distSq <= SAFE_ZONE_SQ then
                triggerDodge(math.max(duration, 1.2))
            -- 2. ĐE DỌA HƯỚNG VÀO VÙNG AN TOÀN TỪ XA (Trong bán kính 300 studs) -> NÉ TỪ XA!
            elseif distSq <= SAFE_DISTANCE_SQ then
