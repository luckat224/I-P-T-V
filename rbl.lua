...
local SkillCategory = {
    ...
    M1          = { bodyHitRange = 10, trackRange = 25,  buffer = 0.1, minDodge = 0.1, height = 100 },
    Melee       = { bodyHitRange = 14, trackRange = 35,  buffer = 0.2, minDodge = 0.2, height = 100 },
    LungeDash   = { bodyHitRange = 16, trackRange = 80,  buffer = 0.3, minDodge = 0.3, height = 100, cone = 120 },
    BeamLine    = { bodyHitRange = 18, trackRange = 100, buffer = 0.5, minDodge = 0.5, height = 120, cone = 120 },
    WideAoE     = { bodyHitRange = 30, trackRange = 55,  buffer = 1.5, minDodge = 1.8, height = 150, isPersistent = true },
...
local function lockCameraToGhost(ghostModel, groundCFrame)
    ...
    local ghostHum = ghostModel and ghostModel:FindFirstChildOfClass("Humanoid")
    if ghostHum then
        pcall(function() camera.CameraSubject = ghostHum end)
        return
    end

    if activeCamFocusPart then pcall(function() activeCamFocusPart:Destroy() end) activeCamFocusPart = nil end
...
local function isGroundLocationSafe(groundCFrame, radius)
    ...
    local regionSize = Vector3.new(radius * 2, 35, radius * 2)
    ...
    local ok, parts = pcall(function() return Workspace:GetPartBoundsInBox(CFrame.new(center), regionSize, overlapParams) end)
    if ok and parts then
        for _, part in ipairs(parts) do
            if part and part.Parent then
                local pName = string.lower(part.Name)
                if pName:find("hit") or pName:find("damage") or pName:find("blast")
                   or pName:find("shockwave") or pName:find("debris") or pName:find("slam")
                   or pName:find("table") or pName:find("flame") or pName:find("tornado")
                   or pName:find("efx") or pName:find("vfx") or pName:find("attack") then
                    return false -- Mặt đất chưa an toàn!
                end

                local charModel = getCharacterFromInstance(part)
                if charModel and charModel ~= dodgeLocalChar then
...
local function triggerDynamicDodge(enemyTrack, enemyChar)
    ...
    -- Cleanup khi hạ cánh bình thường
    if isDodging then
