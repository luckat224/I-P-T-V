...
local function safeLandCharacter(heightOffset, savedCFrame)
    ...
    local targetX = currentPos.X
    local targetZ = currentPos.Z
    ...
    elseif savedCFrame then
        finalPosition = Vector3.new(targetX, savedCFrame.Position.Y, targetZ)
    else
        finalPosition = Vector3.new(targetX, estimatedY, targetZ)
    end

    local finalCFrame = CFrame.new(finalPosition) * (dodgeLocalHrp.CFrame.Rotation)
...
_G.LH_SetInvisible = function(enable)
    if enable then
        ...
        -- LƯU VỊ TRÍ CHUẨN TRƯỚC KHI BAY LÊN
        ...
        -- TELEPORT LÊN SÀN TÀNG HÌNH TRÊN TRỜI
        ...
        RunService:BindToRenderStep("InvisCamAndGhostSync", 202, function()
            -- KIỂM TRA BẢO VỆ NẾU ĐÃ TẮT TÀNG HÌNH HOẶC CHẾT THÌ TỰ UNBIND
            ...
            -- CHO PHÉP NHẢY VÀ DI CHUYỂN TỰ DO TRÊN SÀN ẨN (CHỈ NÍU Y KHI RỚT SÂU DƯỚI SÀN)
            local currentPos = dodgeLocalHrp.Position
            local targetY = invisSavedCFrame.Position.Y + INVIS_HEIGHT
            if currentPos.Y < targetY - 5 then
            ...
            -- GIỮ HIỂN THỊ MÁU NGƯỜI CHƠI KHÁC
        ...
        -- 1. HỦY BỎ BẮT RENDERSTEP TRƯỚC ĐỂ KHÔNG BỊ GIẬT CAMERA/CFRAME
        pcall(function() RunService:UnbindFromRenderStep("InvisCamAndGhostSync") end)

        -- 2. HẠ CÁCH AN TOÀN VỀ ĐẤT
        if invisSavedCFrame then
            safeLandCharacter(INVIS_HEIGHT, invisSavedCFrame)
        end

        -- 3. XÓA MÔ HÌNH VÀ SÀN ẨN
...
lp.CharacterAdded:Connect(function()
    -- RESET HOÀN TOÀN TRẠNG THÁI KHI RESPAWN (TRÁNH LƠ LỬNG TRÊN KHÔNG TRUNG)
...
local function triggerDynamicDodge(enemyTrack, enemyChar)
    ...
    RunService:BindToRenderStep("DodgeCamAndGhostSync", 201, function()
        ...
        -- CHO PHÉP DI CHUYỂN X/Z TỰ DO KHI NÉ, KHOÁ ĐỘ CAO Y TRÊN TRỜI CHỐNG RỚT MAP
        if dodgeLocalHrp and dodgeLocalHrp.Parent and dodgeSavedCFrame then
            local currentPos = dodgeLocalHrp.Position
            local targetY = dodgeSavedCFrame.Position.Y + activeTeleportHeight
            dodgeLocalHrp.CFrame = CFrame.new(currentPos.X, targetY, currentPos.Z) * (dodgeLocalHrp.CFrame.Rotation)
            dodgeLocalHrp.AssemblyLinearVelocity = Vector3.new(dodgeLocalHrp.AssemblyLinearVelocity.X, 0, dodgeLocalHrp.AssemblyLinearVelocity.Z)
        end
