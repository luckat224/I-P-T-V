...
local function hookAnimator(animator, ownerChar)
    ...
    animator.AnimationPlayed:Connect(function(track)
        ...
        if isEnabled then
            ...
            -- CHIÊU DIỆN RỘNG ULTI SAITAMA / METEOR (AoE): Né 100% khi nằm trong bán kính nổ
            if AoESkills[rawId] then
                if dist <= 80 then
                    shouldDodge = true
                end
            else
                -- CHỈ NÉ TỨC THÌ KHI ĐÒN ĐÁNH CỦA ĐỐI THỦ TIẾN VÀO BÁN KÍNH 10 STUDS (VỪA SẮP CHẠM THÂN BẠN)
                if dist <= 10 then
                    shouldDodge = true
                else
                    -- Nếu đối thủ lướt/bắn trực diện hướng thẳng vào mặt bạn và chạm mốc 13 studs: Né ngay!
                    local dirToMe = diff / math.max(dist, 0.001)
                    local enemyLook = enemyHrp.CFrame.LookVector
                    local dot = enemyLook:Dot(dirToMe)

                    if dot >= 0.35 and dist <= 13 then
                        shouldDodge = true
                    end
                end
            end
