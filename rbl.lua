...
local function safeLandCharacter(heightOffset, savedCFrame)
    ...
    local estimatedY = currentPos.Y - heightOffset
...
_G.LH_SetInvisible = function(enable)
    if enable then
        ...
        invisFakePlatform.Position = invisSavedCFrame.Position + Vector3.new(0, INVIS_HEIGHT - legOffset - 2.5, 0)
...
local function emergencyDodgeCleanup()
    if isDodging and dodgeSavedCFrame then
        safeLandCharacter(TELEPORT_HEIGHT, dodgeSavedCFrame)
    end
...
local function triggerDynamicDodge(enemyTrack, enemyChar)
    ...
    fakePlatform.Position = dodgeSavedCFrame.Position + Vector3.new(0, activeTeleportHeight - legOffset - 2.5, 0)
    ...
    while currentTrack and not cancelDodgeSignal do
        ...
        local animDuration = math.max(baseLength + safetyBuffer, 1.2)
