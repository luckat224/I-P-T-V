...
local SkillCategory = {
    ...
    M1          = { bodyHitRange = 10, trackRange = 25,  buffer = 0.1, minDodge = 0.3, height = 100 },
    Melee       = { bodyHitRange = 14, trackRange = 35,  buffer = 0.2, minDodge = 0.4, height = 100 },
    LungeDash   = { bodyHitRange = 20, trackRange = 85,  buffer = 0.3, minDodge = 0.4, height = 100, cone = 120 },
    BeamLine    = { bodyHitRange = 90, trackRange = 120, buffer = 0.5, minDodge = 0.6, height = 120, cone = 120 },
    WideAoE     = { bodyHitRange = 35, trackRange = 75,  buffer = 1.5, minDodge = 1.8, height = 150, isPersistent = true },
    UltimateAoE = { bodyHitRange = 130, trackRange = 180, buffer = 3.5, minDodge = 3.0, height = 350, isPersistent = true },
}
...
local function detectHeroFromCharacter(charModel)
    ...
    for _, child in ipairs(charModel:GetChildren()) do
        ...
        if cName:find("saitama") or cName:find("serious") or cName:find("consecutive") then return "Saitama"
        elseif cName:find("garouv2") or cName:find("cosmic") then return "GarouV2"
        elseif cName:find("garou") or cName:find("flowing water") or cName:find("prey") then return "GarouV1"
        elseif cName:find("cyborg") or cName:find("genos") or cName:find("incinerate") then return "Cyborg"
        elseif cName:find("ninja") or cName:find("sonic") or cName:find("flash strike") then return "Ninja"
        elseif cName:find("metal bat") or cName:find("homerun") or cName:find("savage tornado") then return "MetalBat"
        elseif cName:find("tatsumaki") or cName:find("meteor strike") then return "Tatsumaki"
        elseif cName:find("atomic") or cName:find("sunrise") then return "Samurai"
        elseif cName:find("child emperor") or cName:find("mech beam") then return "ChildEmperor"
        elseif cName:find("zombieman") then return "Zombieman"
        elseif cName:find("suiryu") or cName:find("void tremor") then return "Suiryu"
    ...
    if player and player:FindFirstChild("Backpack") then
        for _, tool in ipairs(player.Backpack:GetChildren()) do
            ...
            if tName:find("saitama") then return "Saitama"
            elseif tName:find("garou v2") or tName:find("cosmic") then return "GarouV2"
            elseif tName:find("garou") then return "GarouV1"
            elseif tName:find("cyborg") or tName:find("genos") then return "Cyborg"
            elseif tName:find("ninja") or tName:find("sonic") then return "Ninja"
            elseif tName:find("metal bat") then return "MetalBat"
            elseif tName:find("tatsumaki") then return "Tatsumaki"
            elseif tName:find("atomic") then return "Samurai"
            elseif tName:find("child emperor") then return "ChildEmperor"
            elseif tName:find("zombieman") then return "Zombieman"
            elseif tName:find("suiryu") then return "Suiryu"
            end
        end
    end
...
local function evaluateDynamicThreat(track, enemyChar, enemyHrp, localHrp)
    ...
    local shouldTrack = not isBodyHit and (dist2D <= trackRange) and isActionAnim
...
local function hookAnimator(animator)
    ...
    connections.AnimationPlayed = animator.AnimationPlayed:Connect(function(track)
        local ownerChar = getCharacterFromInstance(animator)
        if ownerChar then
