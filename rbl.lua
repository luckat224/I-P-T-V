...
local function hookAnimator(animator, ownerChar)
    ...
    animator.AnimationPlayed:Connect(function(track)
        ...
        -- 0. XỬ LÝ CẢNH BÁO DEATH COUNTER SAITAMA (ID 12510170988)
        ...
        if isEnabled then
            ...
            -- CHIÊU DIỆN RỘNG 360 ĐỘ / ULTI (AoE): Né khi thực sự nằm trong BÁN KÍNH BÃO NỔ THỰC TẾ (80 studs)
            if AoESkills[rawId] then
                if dist <= 80 then
                    shouldDodge = true
                end
            else
                -- CHIÊU ĐỊNH HƯỚNG (Đấm, Lướt, Chém, Bắn, Ném Thùng Rác):
                -- CHỈ NÉ KHI TRONG TẦM ĐÁNH TỐI ĐA & ĐỐI THỦ NHẮM MẶT CHÍNH XÁC VÀO THÂN BẠN (dot >= 0.35)
                local maxReach = SkillReach[rawId] or 55
                if dist <= maxReach then
                    local dirToMe = diff / math.max(dist, 0.001)
                    local enemyLook = enemyHrp.CFrame.LookVector
                    local dot = enemyLook:Dot(dirToMe)

                    -- Chỉ né khi đường đạn/hướng vung chiêu của đối thủ thực sự trỏ thẳng vào thân bạn (dot >= 0.35)
                    if dot >= 0.35 then
                        shouldDodge = true
                    end
                end
