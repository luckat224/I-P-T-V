...
local function hookAnimator(animator, ownerChar)
    ...
    animator.AnimationPlayed:Connect(function(track)
        ...
        -- ====================================================================
        -- ⚡ CƠ CHẾ NÉ CẦU THỦ THẦN TỐC (CHỈ NÉ KHI TIẾN VÀO BÁN KÍNH 10 STUDS SẮP CHẠM THÂN) ⚡
        -- ====================================================================
        ...
        if isEnabled then
            ...
            -- THUẬT TOÁN NÉ SIÊU TỐC 0MS - KÍCH HOẠT NGAY LẬP TỨC KHÔNG TRỄ LẠI (PREVENT ALL DAMAGE)
            ...
            -- 1. CHIÊU DIỆN RỘNG (AoE): Né ngay khi trong phạm vi 85 studs
            if AoESkills[rawId] then
                if dist <= 85 then
                    instantDodge = true
                end
            else
                -- 2. CHIÊU ĐỊNH HƯỚNG & M1: Né ngay miligiây đầu tiên (0ms) khi áp sát hoặc chĩa vào bạn
                if dist <= 25 then
                    instantDodge = true
                elseif dist <= 65 and dot >= 0.15 then
                    instantDodge = true
                end
            end
            ...
            if instantDodge then
                ...
                -- Luồng theo dõi thời gian thực dự phòng cho đạn/vật thể di chuyển từ xa
                task.spawn(function()
                    local startTime = os.clock()
                    while track and track.IsPlaying and (os.clock() - startTime < 3.5) and not isDodging do
                        ...
                        local cDiff = cMyPos - cEnemyPos
                        local cDist = cDiff.Magnitude
                        local cDirToMe = cDiff / math.max(cDist, 0.001)
                        local cDot = enemyHrp.CFrame.LookVector:Dot(cDirToMe)

                        if (cDist <= 25) or (cDot >= 0.15 and cDist <= 45) then
                            triggerDynamicDodge(track, ownerChar)
                            break
                        end
