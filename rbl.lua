...
local function triggerDynamicDodge(enemyTrack, enemyChar)
    ...
    -- TELEPORT XÁC THẬT LÊN TRỜI TỨC THÌ 0MS (TRÁNH BỊ DÍNH DAMAGE TRONG FRAME TẠO CLONE)
    ...
    -- ========================================================================
    -- 🧠 TÍNH THỜI GIAN NÉ THÔNG MINH (M1 NÉ NHANH BẬT NẢY 100 STUDS CHO HACKER CHÓNG MẶT)
    -- ========================================================================
    local rawAnimId = enemyTrack and enemyTrack.Animation and string.match(enemyTrack.Animation.AnimationId, "%d+")
    local isM1Attack = rawAnimId and AllM1Skills[rawAnimId]

    local animDuration = 2.0
    if isM1Attack then
        -- M1 ĐẤM TAY: Né cực nhanh 0.22s rồi hạ xuống để M1 tiếp theo giựt lên lại → Hacker bị giựt lên xuống 100 studs liên tục cho chóng mặt!
        animDuration = 0.22
    else
        ...
        animDuration = math.max(baseLength + safetyBuffer, 0.8)
    end

    animDuration = math.min(animDuration, 6)

    local MIN_DODGE_DURATION = isM1Attack and 0.20 or 0.8
    ...
    -- Với Skill thường: Né liền mạch không hạ cánh. Với M1: Giựt xác về để giựt lên tiếp (Hacker chóng mặt)
    if nextDodge and not cancelDodgeSignal and not isM1Attack then
        if fakePlatform then pcall(function() fakePlatform:Destroy() end) fakePlatform = nil end
        if activeGhostModel then pcall(function() activeGhostModel:Destroy() end) activeGhostModel = nil end
        dodgeSavedCFrame = nil
        isDodging = false
...
local function hookAnimator(animator, ownerChar)
    ...
    animator.AnimationPlayed:Connect(function(track)
        ...
        -- CHỈ HỦY NÉ KHI BẠN CHỦ ĐỘNG BẤM SKILL/ĐẤM M1 (KHÔNG HỦY BỞI ANIMATION HỆ THỐNG/ROBLOX MOVEMENT)
        ...
        if isEnabled then
            ...
            -- NÉ 360 ĐỘ TỨC THÌ 0MS - KHÔNG BỊ LẠC HƯỚNG HAY BỊ GIỚI HẠN GÓC LOOKVECTOR
