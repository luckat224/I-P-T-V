...
local SkillCategory = {
    -- bodyHitRange = khoảng cách tối đa chiêu ĐÁNH TRÚNG BODY nhân vật (Generous VIP Range)
    -- trackRange  = khoảng cách theo dõi kẻ địch đang lướt/bắn tới
    M1          = { bodyHitRange = 18, trackRange = 45,  buffer = 0.1, minDodge = 0.3, height = 100 },
    Melee       = { bodyHitRange = 25, trackRange = 60,  buffer = 0.2, minDodge = 0.4, height = 100 },
    LungeDash   = { bodyHitRange = 50, trackRange = 110, buffer = 0.3, minDodge = 0.4, height = 100, cone = 170 },
    BeamLine    = { bodyHitRange = 95, trackRange = 150, buffer = 0.5, minDodge = 0.6, height = 120, cone = 170 },
    WideAoE     = { bodyHitRange = 65, trackRange = 125, buffer = 1.5, minDodge = 1.8, height = 150, isPersistent = true },
    UltimateAoE = { bodyHitRange = 180, trackRange = 250, buffer = 3.5, minDodge = 3.0, height = 350, isPersistent = true },
}
...
local SkillDatabase = {
    ...
    ["10471336737"] = { name = "Consecutive Punches",  hero = "Saitama", category = SkillCategory.Melee, buffer = 1.5, isPersistent = true },
    ...
    ["11343354388"] = { name = "Beatdown Cutscene",    hero = "Saitama", category = SkillCategory.UltimateAoE, isBeatdown = true },
    ...
    ["12296112373"] = { name = "Garou V1 Counter",     hero = "GarouV1", category = SkillCategory.Melee, isDeathCounter = true },
    ["12342416390"] = { name = "Water Stream Rock",    hero = "GarouV1", category = SkillCategory.Melee },
    ["12461556214"] = { name = "Final Hunt Ult",       hero = "GarouV1", category = SkillCategory.UltimateAoE },
    ...
    -- Cyborg (Genos)
    ["12534735382"] = { name = "Machine Gun Blows",   hero = "Cyborg",  category = SkillCategory.Melee, buffer = 1.2, isPersistent = true },
...
-- REAL-TIME GROUND HAZARD SCANNER: Kiểm tra xem vị trí mặt đất có còn Hitbox/Damage/Shockwave/Attack Parts không
local function isGroundLocationSafe(groundCFrame, radius)
    radius = radius or 30
    local center = groundCFrame.Position
    -- Shift search box center upwards (+12 studs) so Y scan range is Y-2 to Y+26 (avoids underground false positives)
    local boxCenter = CFrame.new(center + Vector3.new(0, 12, 0))
    local regionSize = Vector3.new(radius * 2, 28, radius * 2)
    local overlapParams = OverlapParams.new()
    overlapParams.FilterType = Enum.RaycastFilterType.Exclude

    local excludeList = {dodgeLocalChar}
    if activeGhostModel then table.insert(excludeList, activeGhostModel) end
    if invisGhostModel then table.insert(excludeList, invisGhostModel) end
    for _, obj in ipairs(Workspace:GetChildren()) do
        if obj:IsA("BasePart") and (obj.Name:find("Platform") or obj.Name:find("Ghost") or obj.Name == "LH_CamFocusPart") then
            table.insert(excludeList, obj)
        end
    end
    overlapParams.FilterDescendantsInstances = excludeList

    local ok, parts = pcall(function() return Workspace:GetPartBoundsInBox(boxCenter, regionSize, overlapParams) end)
    if ok and parts then
        local checkedChars = {}
        for _, part in ipairs(parts) do
            if part and part.Parent then
                local pName = string.lower(part.Name)
                -- Ignore visual props; require CanTouch or CanCollide or TouchInterest
                local isFunctionalHitbox = part.CanTouch or part.CanCollide or part:FindFirstChildOfClass("TouchTransmitter")
                if isFunctionalHitbox then
                    -- Exclude static map table props (only match hit/damage/shockwave/blast/slam)
                    if pName:find("hitbox") or pName:find("damage") or pName:find("blast")
                       or pName:find("shockwave") or pName:find("slam")
                       or pName:find("flame") or pName:find("tornado")
                       or pName:find("attack") then
                        return false -- Mặt đất chưa an toàn!
                    end
                end

                local charModel = getCharacterFromInstance(part)
                if charModel and charModel ~= dodgeLocalChar and not checkedChars[charModel] then
                    checkedChars[charModel] = true
                    local eHum = charModel:FindFirstChildOfClass("Humanoid")
                    local eAnimator = eHum and eHum:FindFirstChildOfClass("Animator")
                    if eAnimator then
                        local okTr, tracks = pcall(function() return eAnimator:GetPlayingAnimationTracks() end)
                        if okTr and tracks then
                            for _, tr in ipairs(tracks) do
                                if tr.IsPlaying then
                                    local rId = tr.Animation and string.match(tr.Animation.AnimationId, "%d+")
                                    if rId and SkillDatabase[rId] then
                                        return false -- Kẻ địch trên mặt đất vẫn đang tung skill!
                                    end
                                end
                            end
                        end
    ...
    return true -- Mặt đất hoàn toàn sạch sẽ & an toàn!
end

-- ⚡ DYNAMIC REAL-TIME HERO CLASSIFIER (BỎ ID CỐ ĐỊNH - ĐỌC THẲNG DATA GAME) ⚡
local function detectHeroFromCharacter(charModel)
    if not charModel then return "GenericHero" end

    local player = Players:GetPlayerFromCharacter(charModel)
    local attrSources = {charModel, player}

    -- 1. Đọc trực tiếp Attributes real-time trên Character và Player
    for _, src in ipairs(attrSources) do
        if src then
            for _, attrName in ipairs({"Character", "Hero", "Class", "Moveset", "Stance", "Style", "HeroType"}) do
                local val = src:GetAttribute(attrName)
                if val and type(val) == "string" then
                    local lval = string.lower(val)
                    if lval:find("saitama") or lval:find("bald") or lval:find("normal") then return "Saitama"
                    elseif lval:find("garou2") or lval:find("cosmic") or lval:find("garouv2") then return "GarouV2"
                    elseif lval:find("garou") or lval:find("water") or lval:find("hunter") then return "GarouV1"
                    elseif lval:find("cyborg") or lval:find("genos") then return "Cyborg"
                    elseif lval:find("ninja") or lval:find("sonic") then return "Ninja"
                    elseif lval:find("metal") or lval:find("bat") then return "MetalBat"
                    elseif lval:find("tatsumaki") or lval:find("psychic") or lval:find("terrible") then return "Tatsumaki"
                    elseif lval:find("samurai") or lval:find("atomic") then return "Samurai"
                    elseif lval:find("child") or lval:find("emperor") or lval:find("mech") then return "ChildEmperor"
                    elseif lval:find("zombie") then return "Zombieman"
                    elseif lval:find("suiryu") or lval:find("void") then return "Suiryu"
                    elseif lval:find("trash") then return "TrashCan"
                    end
                end
            end
        end
    ...
    -- 2. Quét thực thể trực tiếp trong Character (Tools, Folders, Value, Stance Models)
    for _, child in ipairs(charModel:GetChildren()) do
        local cName = string.lower(child.Name)
        if cName:find("saitama") or cName:find("serious") or cName:find("consecutive") or cName:find("normal punch") or cName:find("shove") or cName:find("uppercut") or cName:find("table flip") or cName:find("beatdown") then return "Saitama"
        elseif cName:find("garouv2") or cName:find("cosmic") then return "GarouV2"
        elseif cName:find("garou") or cName:find("flowing water") or cName:find("lethal whirlwind") or cName:find("hunter") or cName:find("prey") or cName:find("water stream") then return "GarouV1"
        elseif cName:find("cyborg") or cName:find("genos") or cName:find("machine gun") or cName:find("blitz") or cName:find("ignite") or cName:find("incinerate") then return "Cyborg"
        elseif cName:find("ninja") or cName:find("sonic") or cName:find("flash strike") or cName:find("whirlwind kick") or cName:find("scatter") or cName:find("shadow blade") then return "Ninja"
        elseif cName:find("metal bat") or cName:find("homerun") or cName:find("foul ball") or cName:find("grand slam") or cName:find("savage tornado") then return "MetalBat"
        elseif cName:find("tatsumaki") or cName:find("meteor") or cName:find("crushing") or cName:find("boulder") or cName:find("tornado") then return "Tatsumaki"
        elseif cName:find("atomic") or cName:find("samurai") or cName:find("quick slice") or cName:find("cleavage") or cName:find("pinpoint") or cName:find("sunrise") then return "Samurai"
        elseif cName:find("child emperor") or cName:find("mech") or cName:find("gadget") then return "ChildEmperor"
        elseif cName:find("zombieman") or cName:find("regen") then return "Zombieman"
        elseif cName:find("suiryu") or cName:find("vanish") or cName:find("headfirst") or cName:find("dragon fist") or cName:find("void") then return "Suiryu"
        elseif cName:find("trash") then return "TrashCan"
        end
    end

    -- 3. Quét Backpack nếu là player
    if player and player:FindFirstChild("Backpack") then
        for _, tool in ipairs(player.Backpack:GetChildren()) do
            local tName = string.lower(tool.Name)
            if tName:find("saitama") or tName:find("normal punch") or tName:find("shove") or tName:find("uppercut") or tName:find("consecutive") or tName:find("table flip") then return "Saitama"
            elseif tName:find("garou v2") or tName:find("cosmic") then return "GarouV2"
            elseif tName:find("garou") or tName:find("flowing water") or tName:find("lethal whirlwind") or tName:find("hunter") or tName:find("prey") then return "GarouV1"
            elseif tName:find("cyborg") or tName:find("genos") or tName:find("machine gun") or tName:find("blitz") or tName:find("incinerate") then return "Cyborg"
            elseif tName:find("ninja") or tName:find("sonic") or tName:find("flash strike") or tName:find("whirlwind kick") then return "Ninja"
            elseif tName:find("metal bat") or tName:find("homerun") or tName:find("foul ball") or tName:find("savage tornado") then return "MetalBat"
            elseif tName:find("tatsumaki") or tName:find("meteor") or tName:find("boulder") then return "Tatsumaki"
            elseif tName:find("atomic") or tName:find("samurai") or tName:find("quick slice") or tName:find("sunrise") then return "Samurai"
            elseif tName:find("child emperor") or tName:find("mech") then return "ChildEmperor"
            elseif tName:find("zombieman") then return "Zombieman"
            elseif tName:find("suiryu") or tName:find("void") or tName:find("dragon fist") then return "Suiryu"
            end
        end
    end
...
local function isHeroToggleEnabled(heroName)
    if C.AntiHackerOn then return true end
    if heroName == "Saitama" and C.SaitamaOn then return true
    elseif heroName == "GarouV1" and C.GarouV1On then return true
    elseif heroName == "GarouV2" and C.GarouV2On then return true
    elseif heroName == "Cyborg" and C.CyborgOn then return true
    elseif heroName == "Ninja" and C.NinjaOn then return true
    elseif heroName == "TrashCan" and C.TrashCanOn then return true
    elseif heroName == "MetalBat" and C.MetalBatOn then return true
    elseif heroName == "Tatsumaki" and C.TatsumakiOn then return true
    elseif heroName == "Samurai" and C.SamuraiOn then return true
    elseif heroName == "ChildEmperor" and C.ChildEmperorOn then return true
    elseif heroName == "Zombieman" and C.ZombiemanOn then return true
    elseif heroName == "Suiryu" and C.SuiryuOn then return true
    elseif heroName == "GenericHero" then
        -- Chỉ né GenericHero khi có ít nhất 1 nút tướng bất kỳ được bật!
        return C.SaitamaOn or C.GarouV1On or C.GarouV2On or C.CyborgOn or C.NinjaOn 
            or C.TrashCanOn or C.MetalBatOn or C.TatsumakiOn or C.SamuraiOn 
            or C.ChildEmperorOn or C.ZombiemanOn or C.SuiryuOn
    end
    return false
end
...
local function evaluateSkillThreat(rawId, enemyHrp, localHrp)
    local skill = rawId ~= "" and SkillDatabase[rawId]
    ...
    if not skill then
        local isBodyHit = dist2D <= 14 and deltaY <= 30
        local isTrackable = not isBodyHit and dist2D <= 60 and deltaY <= 50
        return isBodyHit, 14, 0.3, 100, 0.4, isTrackable
    end
...
-- ⚡ DYNAMIC THREAT EVALUATION ENGINE (BỎ HOÀN TOÀN PHỤ THUỘC VÀO ID CỐ ĐỊNH) ⚡
local function evaluateDynamicThreat(track, enemyChar, enemyHrp, localHrp)
    local animObj = track and track.Animation
    local rawId = animObj and animObj.AnimationId and string.match(animObj.AnimationId, "%d+") or ""
    
    -- Ưu tiên 1: Dùng thông tin từ database nếu ID có sẵn
    local knownSkill = rawId ~= "" and SkillDatabase[rawId]
    if knownSkill then
        local isThreat, radius, buffer, height, minDodge, shouldTrack = evaluateSkillThreat(rawId, enemyHrp, localHrp)
        return isThreat, radius, buffer, height, minDodge, shouldTrack, knownSkill.hero
    end

    -- Ưu tiên 2: Phân tích 100% Real-Time Game Data (Không cần bất kỳ ID nào trong database!)
    local detectedHero = detectHeroFromCharacter(enemyChar)
    local localPos = (isDodging and dodgeSavedCFrame) and dodgeSavedCFrame.Position or localHrp.Position
    local relPos = localPos - enemyHrp.Position
    local dist2D = math.sqrt(relPos.X * relPos.X + relPos.Z * relPos.Z)
    local deltaY = math.abs(relPos.Y)
    ...
    if deltaY > 100 then
        return false, 15, 0.3, 100, 0.4, false, detectedHero
    end

    local animPriority = track and track.Priority
    local isActionAnim = (animPriority == Enum.AnimationPriority.Action 
                       or animPriority == Enum.AnimationPriority.Action2 
                       or animPriority == Enum.AnimationPriority.Action3 
                       or animPriority == Enum.AnimationPriority.Action4)

    local trackLen = (track and track.Length and track.Length > 0) and track.Length or 1.2
    local animName = animObj and string.lower(animObj.Name or "") or ""

    local category = SkillCategory.Melee
    local isBeatdown = (trackLen > 4.5) or animName:find("beatdown") or animName:find("cutscene") or rawId == "11343318134" or rawId == "11343354388"
    local isUlt = (trackLen > 2.8 and isActionAnim) or animName:find("ult") or animName:find("serious") or animName:find("table") or animName:find("meteor") or animName:find("tornado")

    if isBeatdown or isUlt then
        category = SkillCategory.UltimateAoE
    elseif animName:find("whirlwind") or animName:find("aoe") or animName:find("slam") or animName:find("sweep") or (trackLen > 2.0 and isActionAnim) then
        category = SkillCategory.WideAoE
    elseif animName:find("beam") or animName:find("incinerate") or animName:find("shot") or animName:find("ray") then
        category = SkillCategory.BeamLine
    else
        local vel = enemyHrp.AssemblyLinearVelocity
        local velH = Vector3.new(vel.X, 0, vel.Z)
        if velH.Magnitude > 30 then
            local flatRel = Vector3.new(relPos.X, 0, relPos.Z)
            if flatRel.Magnitude > 0.1 and velH.Unit:Dot(-flatRel.Unit) > 0.5 then
                category = SkillCategory.LungeDash
            end
        end
    end

    local bodyHitRange = category.bodyHitRange
    local trackRange = category.trackRange
    local height = category.height
    local buffer = category.buffer
    local minDodge = category.minDodge

    if category.cone and category.cone < 360 and dist2D > bodyHitRange * 0.5 then
        local flatRel = Vector3.new(relPos.X, 0, relPos.Z)
        local lv = enemyHrp.CFrame.LookVector
        local flatEnemyLook = Vector3.new(lv.X, 0, lv.Z)
        if flatRel.Magnitude > 0.001 and flatEnemyLook.Magnitude > 0.001 then
            local dot = flatEnemyLook.Unit:Dot(flatRel.Unit)
            if dot < math.cos(math.rad(category.cone / 2)) then
                return false, bodyHitRange, buffer, height, minDodge, false, detectedHero
            end
        end
    end

    local isBodyHit = (dist2D <= bodyHitRange) and (deltaY <= 40)
    local shouldTrack = not isBodyHit and (dist2D <= trackRange)

    return isBodyHit, bodyHitRange, buffer, height, minDodge, shouldTrack, detectedHero
end

-- ⚡ ULTRA DYNAMIC REAL-TIME PROBING ENGINE (DÒ GIÂY CHIÊU VÀ TẦM STUDS THỜI GIAN THỰC) ⚡
local function calculateDynamicDodgeParams(track, enemyChar, enemyHrp, localHrp)
    local rawAnimId = track and track.Animation and string.match(track.Animation.AnimationId, "%d+") or ""
    local knownSkill = rawAnimId ~= "" and SkillDatabase[rawAnimId]

    -- 1. DÒ SỐ GIÂY CHIÊU REAL-TIME
    local rawTrackLength = (track and track.Length and track.Length > 0) and track.Length or 1.2
    local trackSpeed = (track and track.Speed and math.abs(track.Speed) > 0.05) and math.abs(track.Speed) or 1.0
    local realTimeSeconds = rawTrackLength / trackSpeed

    -- 2. DÒ TẦM STUDS CHIÊU REAL-TIME
    local localPos = (isDodging and dodgeSavedCFrame) and dodgeSavedCFrame.Position or localHrp.Position
    local relPos = localPos - enemyHrp.Position
    local currentDist2D = math.sqrt(relPos.X * relPos.X + relPos.Z * relPos.Z)

    -- Vận tốc di chuyển của đối thủ (đo lướt / lao tới)
    local eVel = enemyHrp.AssemblyLinearVelocity
    local eHorizontalSpeed = Vector3.new(eVel.X, 0, eVel.Z).Magnitude
    local lungeDistance = eHorizontalSpeed * math.min(realTimeSeconds, 1.5)

    local calculatedStudsReach = 15
    local animName = track and track.Animation and string.lower(track.Animation.Name or "") or ""

    if knownSkill then
        local cat = knownSkill.category
        calculatedStudsReach = cat and cat.bodyHitRange or 15
        if cat == SkillCategory.BeamLine then
            calculatedStudsReach = math.max(calculatedStudsReach, currentDist2D + 20)
        elseif cat == SkillCategory.LungeDash then
            calculatedStudsReach = math.max(calculatedStudsReach, 20 + lungeDistance)
        end
    else
        -- Phân tích tầm Studs từ data game sống
        if realTimeSeconds > 4.0 or animName:find("beatdown") or animName:find("cutscene") or rawAnimId == "11343318134" or rawAnimId == "11343354388" then
            calculatedStudsReach = 130
        elseif realTimeSeconds > 2.5 or animName:find("ult") or animName:find("serious") or animName:find("table") then
            calculatedStudsReach = 100
        elseif animName:find("whirlwind") or animName:find("aoe") or animName:find("slam") or animName:find("sweep") or realTimeSeconds > 1.8 then
            calculatedStudsReach = math.clamp(35 + (realTimeSeconds * 10), 30, 80)
        elseif animName:find("beam") or animName:find("incinerate") or animName:find("shot") or animName:find("ray") then
            calculatedStudsReach = math.clamp(currentDist2D + 25, 40, 150)
        elseif eHorizontalSpeed > 30 then
            calculatedStudsReach = math.clamp(20 + lungeDistance, 25, 90)
        else
            calculatedStudsReach = math.clamp(12 + (realTimeSeconds * 4), 10, 25)
        end
    end

    -- 3. TÍNH TOÁN THỜI GIAN BAY NÉ & ĐỘ CAO NÉ THÔNG MINH
    local isLongChannel = realTimeSeconds >= 2.0
    local maxDodgeSeconds = isLongChannel and math.clamp(realTimeSeconds + 0.3, 1.8, 5.0) or math.clamp(realTimeSeconds + 0.15, 0.3, 2.0)
    local minDodgeSeconds = isLongChannel and math.clamp(realTimeSeconds * 0.65, 0.8, 3.0) or 0.15

    local safeHeightStuds = TELEPORT_HEIGHT
    if calculatedStudsReach >= 90 then
        safeHeightStuds = 350
    elseif calculatedStudsReach >= 35 then
        safeHeightStuds = 150
    end

    return realTimeSeconds, calculatedStudsReach, maxDodgeSeconds, minDodgeSeconds, safeHeightStuds, isLongChannel
end
...
local function safeLandCharacter(heightOffset, savedCFrame)
    if not dodgeLocalChar or not dodgeLocalHrp or not dodgeLocalHum then return end

    local hrpToUnanchor = dodgeLocalHrp
    local humToLand = dodgeLocalHum
    ...
    pcall(function()
        ...
        local currentPos = hrpToUnanchor.Position
        ...
        local legOffset = 3.0
        if humToLand.RigType == Enum.HumanoidRigType.R15 then
            legOffset = humToLand.HipHeight + (hrpToUnanchor.Size.Y / 2)
        else
            legOffset = (humToLand.HipHeight > 0) and (humToLand.HipHeight + 1) or 3.0
        end

        local finalY = highestGroundY and (highestGroundY + legOffset) or expectedY
        local finalCFrame = CFrame.new(targetX, finalY, targetZ) * hrpToUnanchor.CFrame.Rotation

        hrpToUnanchor.Anchored = true
        hrpToUnanchor.CFrame = finalCFrame
        hrpToUnanchor.AssemblyLinearVelocity = Vector3.zero
        hrpToUnanchor.AssemblyAngularVelocity = Vector3.zero
        if humToLand and humToLand.Health > 0 then
            humToLand:ChangeState(Enum.HumanoidStateType.Running)
        end
    end)

    -- GUARANTEED UNANCHORING REGARDLESS OF ERRORS
    if hrpToUnanchor and hrpToUnanchor.Parent then
        pcall(function()
            hrpToUnanchor.Anchored = false
            hrpToUnanchor.AssemblyLinearVelocity = Vector3.zero
            hrpToUnanchor.AssemblyAngularVelocity = Vector3.zero
        end)
    end
...
local function handleSaitamaBeatdownEscape(enemyChar)
    ...
    local BEATDOWN_ESCAPE_HEIGHT = 100000 -- Độ cao cực đại (100,000 studs) lên tận bầu khí quyển
    ...
    local heightOffsetVec = Vector3.new(0, BEATDOWN_ESCAPE_HEIGHT, 0)
    camera.CFrame = camera.CFrame - heightOffsetVec

    local escapeStart = os.clock()
    task.spawn(function()
        while (os.clock() - escapeStart < 7.5) do
            if not dodgeLocalHum or dodgeLocalHum.Health <= 0 or not dodgeLocalHrp or not dodgeLocalHrp.Parent then break end
            
            if math.abs(dodgeLocalHrp.Position.Y - (currentCFrame.Position.Y + BEATDOWN_ESCAPE_HEIGHT)) > 5 then
                dodgeLocalHrp.CFrame = CFrame.new(dodgeLocalHrp.Position.X, currentCFrame.Position.Y + BEATDOWN_ESCAPE_HEIGHT, dodgeLocalHrp.Position.Z)
                dodgeLocalHrp.AssemblyLinearVelocity = Vector3.zero
            end
            ...
            if enemyChar then
                ...
                local eHrp = getRootPart(enemyChar)
                if not enemyChar.Parent or (eHum and eHum.Health <= 0) or (eHrp and eHrp.Position.Y < 0) then
                    break
                end
            end
...
-- Multi-Layer Enterprise Anti-Hacker Engine (Optimized & Zero-Stutter)
local antiHackerSteppedConn = nil
local antiHackerHeartbeatConn = nil
local antiHackerHitboxConn = nil
...
_G.LH_SetAntiHacker = function(enable)
    ...
    if antiHackerHitboxConn then pcall(function() antiHackerHitboxConn:Disconnect() end) antiHackerHitboxConn = nil end
    
    lastValidCFrame = nil
    if not enable then return end

    antiHackerSteppedConn = RunService.Stepped:Connect(function(time, dt)
        pcall(function()
            ...
            -- Phase Collision Disabling for Enemy Characters (Lightweight Iteration)
            ...
            -- Anti-Weld Bring Protection: Destroy external constraint attachments to local character
            for _, obj in ipairs(myChar:GetDescendants()) do
                if obj:IsA("Weld") or obj:IsA("ManualWeld") or obj:IsA("WeldConstraint") or obj:IsA("RopeConstraint") 
                   or obj:IsA("Motor6D") or obj:IsA("AlignPosition") or obj:IsA("LinearVelocity") or obj:IsA("Seat") then
                    
                    local p0 = (obj:IsA("Weld") or obj:IsA("ManualWeld") or obj:IsA("WeldConstraint") or obj:IsA("Motor6D")) and obj.Part0 or nil
                    local p1 = (obj:IsA("Weld") or obj:IsA("ManualWeld") or obj:IsA("WeldConstraint") or obj:IsA("Motor6D")) and obj.Part1 or nil
                    ...
                    elseif obj:IsA("Seat") and obj.Parent == myChar then
                        isForeignBring = true
                    elseif obj:IsA("RopeConstraint") or obj:IsA("AlignPosition") then
            ...
            -- Anti-Fling Velocity Clamping across character assembly
            local maxLinVel = math.max(250, (C.SpeedOn and C.Speed * 3.0 or 180))
            local isLinearFling = hrp.AssemblyLinearVelocity.Magnitude > maxLinVel
            local isAngularFling = hrp.AssemblyAngularVelocity.Magnitude > 120
            ...
            if currentPos.Y < (destroyHeight + 60) then
                hrp.AssemblyLinearVelocity = Vector3.zero
                hrp.AssemblyAngularVelocity = Vector3.zero
            ...
            -- Speed Validation & Distance Guard with Horizontal 2D Distance & State Exclusions
            local hState = hum:GetState()
            local isCustomMoving = isDodging or beatdownRunning or C.InvisibleOn or C.FlyOn or C.NoclipOn or C.Tweening or C.Teleporting 
                or hState == Enum.HumanoidStateType.PlatformStanding 
                or hState == Enum.HumanoidStateType.Seated 
                ...
                or hState == Enum.HumanoidStateType.Physics 
                or hum.Sit

            if lastValidCFrame and not isCustomMoving then
                local currentVelH = Vector3.new(hrp.AssemblyLinearVelocity.X, 0, hrp.AssemblyLinearVelocity.Z).Magnitude
                local expectedSpeed = math.max((C.SpeedOn and C.Speed or 16), currentVelH)
                local maxAllowedDist2D = (expectedSpeed * dt) + 40

                local delta2D = Vector3.new(currentPos.X - lastValidCFrame.Position.X, 0, currentPos.Z - lastValidCFrame.Position.Z).Magnitude
                if delta2D > maxAllowedDist2D then
    ...
    -- ⚡ HITBOX DETECTION SYSTEM: Clean Connection Scoped to Module
    antiHackerHitboxConn = Workspace.DescendantAdded:Connect(function(obj)
        if not C.AntiHackerOn then return end
        ...
        if (obj.Transparency >= 0.8 or not obj.CanCollide) and obj.Size.Magnitude < 100 then
            ...
            if lname:find("hit") or lname:find("damage") or lname:find("attack")
               ...
               or lname:find("projectile") or lname:find("blast") or lname:find("hb") then
                isLikelyHitbox = true
            end
        end
        ...
        -- Immediate zero-delay spatial validation
        ...
        if dist2D <= 25 and deltaY <= 35 then
...
local function triggerDynamicDodge(enemyTrack, enemyChar)
    ...
    local enemyHrpInst = enemyChar and getRootPart(enemyChar)
    if not enemyHrpInst then return end

    -- ⚡ DÒ SỐ GIÂY CHIÊU VÀ TẦM STUDS REAL-TIME ⚡
    local probedSeconds, probedReach, maxDodgeSeconds, minDodgeSeconds, safeHeightStuds, isLongChannel = calculateDynamicDodgeParams(enemyTrack, enemyChar, enemyHrpInst, dodgeLocalHrp)
    activeDodgeHeight = safeHeightStuds

    task.delay(math.clamp(maxDodgeSeconds + 1.0, 3.0, 7.0), function()
    ...
    local ghostRealParts = rParts or {}
    local ghostCloneParts = gParts or {}
    local ghostPartCount = #ghostCloneParts

    pcall(function() RunService:UnbindFromRenderStep("DodgeCamAndGhostSync") end)

    -- Teleport HRP lên cao
    dodgeLocalHrp.CFrame = dodgeSavedCFrame + Vector3.new(0, activeDodgeHeight, 0)
    dodgeLocalHrp.AssemblyLinearVelocity = Vector3.zero
    dodgeLocalHrp.AssemblyAngularVelocity = Vector3.zero

    -- Offset Camera CFrame ngay từ frame đầu tiên (ĐỒNG BỘ 100% GIỐNG LÚC BẬT TÀNG HÌNH!)
    local initialHVec = Vector3.new(0, activeDodgeHeight, 0)
    camera.CFrame = camera.CFrame - initialHVec

    -- Render Priority: Camera.Value + 1 (2001) chạy NGAY SAU Roblox Camera Script để offset CFrame mượt 100%
    local CAM_PRIORITY = Enum.RenderPriority.Camera.Value + 1
    ...
    RunService:BindToRenderStep("DodgeCamAndGhostSync", CAM_PRIORITY, function()
        ...
        -- Dynamic Camera Offset & Ghost Sync (100% GIỐNG TÀNG HÌNH!)
        local hVec = Vector3.new(0, activeDodgeHeight, 0)
        camera.CFrame = camera.CFrame - hVec

        if ghostRealParts and ghostCloneParts then
            for i = 1, ghostPartCount do
                local rp = ghostRealParts[i]
                local gp = ghostCloneParts[i]
                if rp and gp and rp.Parent and gp.Parent then
                    gp.CFrame = rp.CFrame - hVec
                end
            end
        end
    end)
    ...
    while currentTrack and not cancelDodgeSignal do
        ...
        while not cancelDodgeSignal and (os.clock() - startTime < maxDodgeSeconds) do
            ...
            -- ⚡ REAL-TIME DURATION PROBING: Cho chiêu ngắn/M1, thoát né NGAY LẬP TỨC khi animation kết thúc hoặc tới 80% thời gian
            if not isLongChannel then
                if trackEnded or (currentTrack and not currentTrack.IsPlaying) or (currentTrack and currentTrack.Length > 0 and currentTrack.TimePosition >= currentTrack.Length * 0.80) then
                    break
                end
            end

            -- ⚡ REAL-TIME STUDS REACH CHECK: Kiểm tra khoảng cách thực tế so với Tầm Studs đã đo được
            if dodgeSavedCFrame and dodgeLocalHrp and (elapsed >= minDodgeSeconds) then
                ...
                if enemyHrpSafe and enemyHrpSafe.Parent then
                    ...
                    if dist2DSafe > (probedReach * 1.5) then
                        cancelDodgeSignal = true
                        break
                    end
                ...
                -- ⚡ GROUND HAZARD CHECK CHO CHIÊU DIỆN RỘNG
                if isLongChannel then
                    if isGroundLocationSafe(dodgeSavedCFrame, math.min(probedReach, 40)) then
                        break
                    end
                end
            end
        ...
        if currentTrack then
            local nHrp = dodgeEnemyChar and getRootPart(dodgeEnemyChar)
            if nHrp then
                local _, _, nMaxSec, nMinSec, nHeight, nLong = calculateDynamicDodgeParams(currentTrack, dodgeEnemyChar, nHrp, dodgeLocalHrp)
                activeDodgeHeight = nHeight
                maxDodgeSeconds = nMaxSec
                minDodgeSeconds = nMinSec
                isLongChannel = nLong
            end
        end
    end

    -- Atomic Teardown: Land HRP first, then unbind & destroy ghost objects (100% ĐỒNG BỘ TÀNG HÌNH)
    if isDodging then
    ...
    while current and current ~= Workspace do
        if current:IsA("Model") then
            if current:FindFirstChildOfClass("Humanoid") 
               ...
               or Players:GetPlayerFromCharacter(current) then
                return current
            end
        end
        ...
        if not ownerChar or not ownerChar.Parent
            ...
            if enemyHrp and enemyHrp.Parent then
                ...
                if passDirectionCheck then
                    local isBodyHit = evaluateDynamicThreat(track, ownerChar, enemyHrp, myHrp)
                    if isBodyHit then
                        table.remove(ActiveTrackingTasks, i)
                        task.spawn(function()
...
local function onAnimationPlayed(track, animator, boundChar)
    ...
    if realOwnerChar == myChar or realOwnerChar:IsDescendantOf(myChar) then
        ...
        -- Cancel dodge INSTANTLY on ANY local player animation (M1, skill, punch)
        if isDodging then
            cancelDodgeNow()
        end
        return
    ...
    if (rawId == "11343318134" or rawId == "11343354388") and (C.SaitamaOn or C.AntiHackerOn) then
    ...
    if rawId == "12447707844" and (C.SaitamaOn or C.AntiHackerOn) then
    ...
    -- ⚡ REAL-TIME GAME DATA THREAT EVALUATION (ZERO HARDCODED ID RELIANCE)
    local isThreat, radius, buffer, height, minDodge, shouldTrack, detectedHero = evaluateDynamicThreat(track, realOwnerChar, enemyHrp, myHrp)

    if not isHeroToggleEnabled(detectedHero) then return end
...
local function hookAnimator(animator)
    ...
    local function cleanup()
        if not AnimatorTracker[animator] then return end
    ...
    connections.AnimationPlayed = animator.AnimationPlayed:Connect(function(track)
        local ownerChar = getCharacterFromInstance(animator) or boundChar or (animator.Parent and animator.Parent.Parent)
        if ownerChar and ownerChar:IsA("Humanoid") then ownerChar = ownerChar.Parent end
        if ownerChar then
            onAnimationPlayed(track, animator, ownerChar)
        end
    end)
    ...
    if ok and tracks then
        for _, track in ipairs(tracks) do
            if track.IsPlaying then
                ...
                if ownerChar and ownerChar:IsA("Humanoid") then ownerChar = ownerChar.Parent end
                if ownerChar then
                    task.spawn(onAnimationPlayed, track, animator, ownerChar)
                end
            end
        end
    end
    ...
    connections.Destroying = animator.Destroying:Connect(cleanup)
...
local function scanAllGameAnimators()
...
scanAllGameAnimators()

Workspace.DescendantAdded:Connect(function(obj)
    if obj:IsA("Animator") then
        hookAnimator(obj)
    elseif obj:IsA("Model") then
        if obj:FindFirstChildOfClass("Humanoid") or Players:GetPlayerFromCharacter(obj) then
            scanContainer(obj)
        end
    end
end)
...
Players.PlayerAdded:Connect(function(player)
    player.CharacterAdded:Connect(function(char)
        task.delay(0.5, function() scanContainer(char) end)
    end)
end)
