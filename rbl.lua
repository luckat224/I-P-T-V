...
local function triggerDynamicDodge(enemyTrack, enemyChar)
    ...
    if enemyTrack and enemyTrack.Length and enemyTrack.Length > 0 then
        animDuration = enemyTrack.Length
    end

    local startTime = os.clock()
    -- TRỐN TRÊN TRỜI CHO ĐẾN KHI CHIÊU CỦA ĐỐI THỦ THỰC THỰC SỰ HẾT SỐ GIÂY (FULL ANIMATION DURATION)
    while not cancelDodgeSignal and not trackEnded and (enemyTrack and enemyTrack.IsPlaying) and (os.clock() - startTime < animDuration + 0.15) do
...
local function hookAnimator(animator, ownerChar)
    ...
    animator.AnimationPlayed:Connect(function(track)
        ...
        -- ====================================================================
        -- ⚡ CƠ CHẾ NÉ CẦU THỦ THẦN TỐC (CỐ ĐỊNH 2D LOẠI BỎ LỖI KHI ĐANG NHẢY TRÊN KHÔNG) ⚡
        -- ====================================================================
        ...
        if isEnabled then
            ...
            -- HỆ THỐNG 360-DEGREE DYNAMIC SAFE ZONE OVERLAP ENGINE (SIÊU TỐC THẦN TỐC 0MS - FIX TRONG KHI NHẢY)
            local SAFE_ZONE_RADIUS = 15

            local myPos = dodgeLocalHrp.Position
            local enemyPos = enemyHrp.Position

            -- CHÍNH XÁC 100%: TÍNH KHOẢNG CÁCH 2D MẶT PHẲNG NẰM NGANG (TRIỆT TIÊU LỖI BỊ ẢNH HƯỞNG KHI ĐANG NHẢY TRÊN KHÔNG)
            local diff2D = Vector3.new(myPos.X - enemyPos.X, 0, myPos.Z - enemyPos.Z)
            local dist = diff2D.Magnitude

            local attackRadius = 18
            if AoESkills[rawId] then
                attackRadius = 75
            elseif NinjaSkills[rawId] then
                attackRadius = 55 -- NẮM BẮT TỐC ĐỘ NINJA TỰ ĐỘNG THẦN TỐC
            elseif AllM1Skills[rawId] then
                attackRadius = 14
            end

            local maxOverlapDistance = SAFE_ZONE_RADIUS + attackRadius

            local dirToMe = diff2D.Unit
            local enemyLook2D = Vector3.new(enemyHrp.CFrame.LookVector.X, 0, enemyHrp.CFrame.LookVector.Z).Unit
            local dot = enemyLook2D:Dot(dirToMe)

            local instantDodge = false

            -- Né 360 độ tức thì bất kể đằng sau lưng, bên hông hay đang nhảy trên không
            if dist <= maxOverlapDistance then
                instantDodge = true
            elseif dist <= 85 and dot >= 0.1 then
                instantDodge = true
            end

            if instantDodge then
                task.spawn(function()
                    triggerDynamicDodge(track, ownerChar)
                end)
            else
                -- Luồng theo dõi thời gian thực dự phòng cho chiêu đạn / tên lửa / lốc xoáy lướt tới từ xa
                task.spawn(function()
                    local startTime = os.clock()
                    while track and track.IsPlaying and (os.clock() - startTime < 3.5) and not isDodging do
                        ...
                        local cDiff2D = Vector3.new(cMyPos.X - cEnemyPos.X, 0, cMyPos.Z - cEnemyPos.Z)
                        local cDist = cDiff2D.Magnitude
                        local cDirToMe = cDiff2D.Unit
                        local cEnemyLook2D = Vector3.new(enemyHrp.CFrame.LookVector.X, 0, enemyHrp.CFrame.LookVector.Z).Unit
                        local cDot = cEnemyLook2D:Dot(cDirToMe)

                        if (cDist <= maxOverlapDistance) or (cDot >= 0.1 and cDist <= 75) then
                            triggerDynamicDodge(track, ownerChar)
                            break
                        end
