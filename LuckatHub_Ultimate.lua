...
local function makeTab(id, label)
    ...
    -- Content panel (outer clip frame - NO ScrollingFrame)
    ...
    -- Inner scrollable frame (moved by scroll buttons)
    local inner = Instance.new("Frame")
    inner.Name = "Inner"
    inner.Size = UDim2.new(1, -8, 0, 2000) -- tall enough for all widgets
    ...
    scrollDown.Size = UDim2.new(0, 18, 0, 60)
    scrollDown.Position = UDim2.new(1, -20, 0.5, -30)
    scrollDown.BackgroundColor3 = Color3.fromRGB(40, 40, 60)
    scrollDown.Text = "v"
    scrollDown.TextColor3 = Color3.fromRGB(180, 180, 200)
    scrollDown.TextSize = 11
    scrollDown.Font = Enum.Font.SourceSansBold
    scrollDown.BorderSizePixel = 0
    scrollDown.ZIndex = 20
    scrollDown.Parent = panel
    local sdc = Instance.new("UICorner")
    sdc.CornerRadius = UDim.new(0, 4)
    sdc.Parent = scrollDown

    -- Scroll UP button
    local scrollUp = Instance.new("TextButton")
    scrollUp.Size = UDim2.new(0, 18, 0, 60)
    scrollUp.Position = UDim2.new(1, -20, 0.5, -94)
    scrollUp.BackgroundColor3 = Color3.fromRGB(40, 40, 60)
    scrollUp.Text = "^"
    scrollUp.TextColor3 = Color3.fromRGB(180, 180, 200)
    scrollUp.TextSize = 11
    scrollUp.Font = Enum.Font.SourceSansBold
    scrollUp.BorderSizePixel = 0
    scrollUp.ZIndex = 20
    scrollUp.Parent = panel
    ...
    scrollDown.MouseButton1Click:Connect(function()
        ...
        local innerH = list.AbsoluteContentSize.Y + 12
        local maxScroll = -(math.max(innerH - panelH, 0))
        local newY = math.max(inner.Position.Y.Offset - SCROLL_STEP, maxScroll)
        inner.Position = UDim2.new(0, 4, 0, newY)
    end)
    scrollUp.MouseButton1Click:Connect(function()
        local newY = math.min(inner.Position.Y.Offset + SCROLL_STEP, MIN_Y)
        inner.Position = UDim2.new(0, 4, 0, newY)
    end)
    ...
    return inner -- return inner Frame so widgets are added to it
end
