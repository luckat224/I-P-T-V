-- LuckatHub Ultimate v7.0 - MINIMAL ARCHITECTURE REBUILD
-- Xây dựng lại hoàn toàn: KHÔNG dùng ScrollingFrame (gây đen trắng trên Delta Android)
-- Dùng Frame thuần + UIListLayout + CanvasGroup thay thế
...
pcall(function()
    for _, v in ipairs(safeParent:GetChildren()) do
        if string.find(v.Name, "Luckat") then v:Destroy() end
    end
end)
...
local function getChar()
    ...
    if charCache.char then
        charCache.hum = charCache.char:FindFirstChildOfClass("Humanoid")
        charCache.hrp = charCache.char:FindFirstChild("HumanoidRootPart")
    end
end
getChar()
...
local function draggable(win, handle)
    local drag, startP, startWP = false
    handle.InputBegan:Connect(function(i)
        if i.UserInputType == Enum.UserInputType.Touch or i.UserInputType == Enum.UserInputType.MouseButton1 then
            drag=true; startP=i.Position; startWP=win.Position
        end
    end)
    ...
    UIS.InputChanged:Connect(function(i)
        ...
        if i.UserInputType == Enum.UserInputType.Touch or i.UserInputType == Enum.UserInputType.MouseMovement then
            ...
            win.Position = UDim2.new(startWP.X.Scale, startWP.X.Offset+d.X, startWP.Y.Scale, startWP.Y.Offset+d.Y)
        end
    end)
end

-- ====== BUILD GUI ======
-- Dùng ZIndexBehavior.Global + không dùng ScrollingFrame + tất cả ZIndex tường minh
...
winStroke.Color = Color3.fromRGB(110,35,200)
...
-- Content area - FRAME THUẦN (không ScrollingFrame)
...
-- ====== TAB SYSTEM ======
local tabPanels = {}   -- id -> Frame
local tabBtns   = {}   -- id -> TextButton
local activeTab = ""

local ACTIVE_BG   = Color3.fromRGB(100,30,185)
local INACTIVE_BG = Color3.fromRGB(24,26,38)
local ACTIVE_TXT  = Color3.new(1,1,1)
local INACTIVE_TXT= Color3.fromRGB(160,165,185)
...
local function switchTab(id)
    ...
    for tid, btn in pairs(tabBtns) do
        local on = (tid == id)
        tween(btn, {BackgroundColor3 = on and ACTIVE_BG or INACTIVE_BG})
        btn.TextColor3 = on and ACTIVE_TXT or INACTIVE_TXT
    end
end
...
local function makeTab(id, label)
    ...
    -- Content panel - FRAME thường không ScrollingFrame
    ...
    -- Dùng ScrollingFrame NẰM TRONG panel để cuộn, với ZIndex rõ ràng
    ...
    local p2 = Instance.new("UIPadding",sf)
    p2.PaddingAll = UDim.new(0,7)

    tabPanels[id] = panel
    tabBtns[id]   = btn

    btn.MouseButton1Click:Connect(function() switchTab(id) end)
    return sf  -- trả về ScrollingFrame để add widget vào
end

-- ====== WIDGET BUILDERS ======
local CARD_BG = Color3.fromRGB(22,24,36)
...
local function addLabel(sf, txt)
    ...
    l.Text = txt
    l.TextColor3 = Color3.fromRGB(130,135,160)
    l.TextSize = 11
...
local function addSwitch(sf, label, default, onChange)
    ...
    track.MouseButton1Click:Connect(function()
        state = not state
        tween(track,{BackgroundColor3=state and ON_COL or OFF_COL})
        tween(knob,{Position=state and UDim2.new(1,-13,0.5,-5.5) or UDim2.new(0,2,0.5,-5.5)})
...
local function addNum(sf, label, val, mn, mx, onChange)
    ...
    box.FocusLost:Connect(function()
        ...
        if n then n=math.clamp(n,mn,mx); box.Text=tostring(n); pcall(onChange,n)
...
local function addBtn(sf, label, col, onClick)
    ...
    btn.BackgroundColor3 = col or Color3.fromRGB(100,30,185)
    btn.Text = label
...
addSwitch(t2,"Wallhack ESP",CFG.WallhackOn,function(v)
    ...
    for _,p in ipairs(Players:GetPlayers()) do
        if p.Character then
            local hl = p.Character:FindFirstChild("LHwallhack")
            if hl then hl.Enabled=v end
        end
    end
end)
addLabel(t2, "--- CHIEN DAU ---")
addSwitch(t2,"Aimbot (Nham muc tu dong)",CFG.AimbotOn,function(v) CFG.AimbotOn=v end)
addSwitch(t2,"Hien Nut HUD Tren Man Hinh",CFG.OnScreenBtns,function(v)
    CFG.OnScreenBtns=v
    -- trigger HUD toggle (defined later)
    if _G.LH_toggleHUD then _G.LH_toggleHUD(v) end
end)

-- ====== TAB 3: BAO VE ======
addLabel(t3, "--- SONG SOT ---")
addSwitch(t3,"Anti-AFK",CFG.AntAFKOn,function(v) CFG.AntAFKOn=v end)
...
addSwitch(t3,"Ghost Mode (Bat tat toan bo)",CFG.GhostOn,function(v) CFG.GhostOn=v end)
addSwitch(t3,"Phantom Velocity (Vang khi bi ap)",CFG.PhantomOn,function(v) CFG.PhantomOn=v end)
addNum(t3,"Phantom Force",CFG.PhantomForce,50,200,function(v) CFG.PhantomForce=v end)

-- ====== TAB 4: ULTRA BYPASS ======
addLabel(t4, "--- BAO VE KHOI KICK/BAN ---")
addSwitch(t4,"Kick Block (Chong bi kick)",true,function(v)
...
addBtn(t4,"Scan & Yield AC Threads",Color3.fromRGB(160,40,40),function()
    pcall(function()
        if not getthreads then print("getthreads() khong kha dung"); return end
        local count=0
        ...
        print("[LuckatHub] Yielded "..count.." AC thread(s)")
    end)
end)
addLabel(t4, "--- TRANG THAI ---")
local statusLbl = Instance.new("TextLabel")
statusLbl.Size = UDim2.new(1,-4,0,26)
statusLbl.BackgroundColor3 = Color3.fromRGB(15,38,20)
...
addBtn(t5,"Bat Fix Lag Live",Color3.fromRGB(0,160,110),function()
    if _G.LH_fixLagLive then _G.LH_fixLagLive() end
end)
...
task.spawn(function()
    ...
    RunService.Heartbeat:Connect(function()
        ...
        if CFG.JumpOn then hum.UseJumpPower=true; hum.JumpPower=CFG.JumpPow end
        ...
        if CFG.NoclipOn and charCache.char then
            for _,p in ipairs(charCache.char:GetDescendants()) do
                if p:IsA("BasePart") then p.CanCollide=false end
            end
        end

        if CFG.AntiStunOn and charCache.char then
            hum.PlatformStand=false
            for _,c in ipairs(charCache.char:GetChildren()) do
                local n=c.Name
                if n=="Stun" or n=="Freeze" or n=="Ragdoll" or n=="Knockback" then pcall(function() c:Destroy() end) end
            end
        end
    ...
    RunService.RenderStepped:Connect(function()
        local hrp=charCache.hrp; local hum=charCache.hum
        ...
        if CFG.FlyOn then
            ...
            if not flyBG or not flyBG.Parent then
                flyBG=Instance.new("BodyGyro"); flyBG.MaxTorque=Vector3.new(1e5,1e5,1e5); flyBG.P=5e4; flyBG.CFrame=hrp.CFrame; flyBG.Parent=hrp
            end
...
task.spawn(function()
    lp.Idled:Connect(function()
        if not CFG.AntAFKOn then return end
        pcall(function()
...
-- ====== SUBSYSTEM: WALLHACK ESP ======
task.spawn(function()
    local function applyHL(char,p)
        pcall(function()
            local old=char:FindFirstChild("LHwallhack")
    ...
    local function setupP(p)
        ...
        if p.Character then applyHL(p.Character,p) end
        p.CharacterAdded:Connect(function(c) task.wait(0.5); applyHL(c,p) end)
    end
...
task.spawn(function()
    ...
    RunService.Heartbeat:Connect(function()
        local hrp=charCache.hrp; local hum=charCache.hum
        if not hrp or not hum or not CFG.GhostOn then
            for _,p in ipairs(Players:GetPlayers()) do
                if p~=lp and p.Character then
                    local r=p.Character:FindFirstChild("HumanoidRootPart")
                    if r then prevPos[p]=r.Position end
                end
            end
            return
        end
            ...
            if p~=lp and p.Character then
                ...
                if r then
                    ...
                    if prev then
                        ...
                        if jumped>60 and dist<15 and now-dodgeCD>0.4 then
                            dodgeCD=now
...
task.spawn(function()
    ...
    pcall(function()
        ...
        local oldNC=hookmetamethod(game,"__namecall",function(self,...)
            ...
            if kickBlock and (m=="Kick" or m=="Ban") then
                warn("[LH Bypass] "..m.." blocked"); return nil
            end
            if remFilter and (m=="FireServer" or m=="InvokeServer") then
                ...
                if ok and n then
                    for _,kw in ipairs({"report","cheat","hack","detect","ban","kick","anticheat","sanity","exploit","flag"}) do
                        if n:find(kw,1,true) then warn("[LH Bypass] Remote blocked: "..self.Name); return nil end
                    end
                end
            end
            return oldNC(self,...)
    ...
    if statusLbl then
        statusLbl.Text="KickBlock: ON | RemoteFilter: ON | Hook: "..(hooked and "OK" or "Safe Mode")
    end
end)

-- ====== SUBSYSTEM: ANTI-HACKER TP ======
task.spawn(function()
    local prevPos={}
    local warnCD={}
    RunService.Heartbeat:Connect(function()
        if not CFG.AntiHackOn then
            for _,p in ipairs(Players:GetPlayers()) do
                if p~=lp and p.Character then
                    local r=p.Character:FindFirstChild("HumanoidRootPart")
                    if r then prevPos[p]=r.Position end
            ...
            if p~=lp and p.Character then
                ...
                if r then
                    ...
                    if prev then
                        ...
                        local dist=(cur-myPos).Magnitude
                        local now=tick()
                        if jumped>80 and dist<CFG.AntiHackRadius and (not warnCD[p] or now-warnCD[p]>2) then
                            warnCD[p]=now
                            local pushDir=(myPos-cur)
                            if pushDir.Magnitude>0 then pushDir=pushDir.Unit else pushDir=-camera.CFrame.LookVector end
                            myHRP.CFrame=CFrame.new(myPos+pushDir*CFG.AntiHackPush)
                            myHRP.AssemblyLinearVelocity=pushDir*25
                        end
                    end
                    prevPos[p]=cur
                end
...
local function stripFX(o) if FX[o.ClassName] then o.Enabled=false end end

_G.LH_fixLagLive=function()
    if fixConn then return end
    for _,o in ipairs(Workspace:GetDescendants()) do pcall(stripFX,o) end
...
_G.LH_toggleHUD=function(on)
    ...
    -- Teleport logic
    local locked=false; local lockTarget=nil; local followConn=nil
    ...
    local function tpTo(target)
        ...
        local rv=tr.CFrame.RightVector
        pr.CFrame=CFrame.new(tr.Position+rv*2, tr.Position)
    end

    tpBtn.MouseButton1Click:Connect(function()
        if locked then
            locked=false; lockTarget=nil
            if followConn then followConn:Disconnect(); followConn=nil end
            tpBtn.BackgroundColor3=Color3.fromRGB(220,50,50); tpBtn.Text="TELEPORT"
        else
            local t=getTarget()
            if not t then tpBtn.Text="NO TARGET"; task.delay(1,function() tpBtn.Text="TELEPORT" end); return end
            locked=true; lockTarget=t
            tpBtn.BackgroundColor3=Color3.fromRGB(0,200,80); tpBtn.Text="LOCKED"
            tpTo(t)
            ...
            followConn=RunService.Heartbeat:Connect(function()
                if not locked or not lockTarget or not lockTarget.Character then
                    if followConn then followConn:Disconnect(); followConn=nil end
                    locked=false; lockTarget=nil
                    if tpBtn.Parent then tpBtn.BackgroundColor3=Color3.fromRGB(220,50,50); tpBtn.Text="TELEPORT" end
                    return
                end
                tpTo(lockTarget)
            end)
...
print("[LuckatHub v7.0] Load thanh cong - 100% Delta Mobile Android!")
