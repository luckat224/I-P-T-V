...
-- Hàm xử lý kéo thanh trượt tốc độ
local function onSpeedSliderInput(input)
    if input.UserInputType == Enum.UserInputType.MouseButton1 then
        isDraggingSpeed = true
    end
end

local function onSpeedSliderInputChanged(input)
    if isDraggingSpeed and input.UserInputType == Enum.UserInputType.MouseMovement then
        ...
        local relativeX = (input.Position.X - sliderAbsolutePos.X) - knobWidth / 2
        local availableWidth = sliderAbsoluteSize.X - knobWidth
        
        -- Giới hạn vị trí trong phạm vi thanh trượt
        relativeX = math.clamp(relativeX, 0, availableWidth)
        
        -- Tính giá trị tốc độ
        local normalizedValue = relativeX / availableWidth
...
local function onSpeedSliderInputEnded(input)
    if input.UserInputType == Enum.UserInputType.MouseButton1 then
...
-- Kết nối sự kiện thanh trượt tốc độ
speedKnob.InputBegan:Connect(onSpeedSliderInput)
speedKnob.InputChanged:Connect(onSpeedSliderInputChanged)
speedKnob.InputEnded:Connect(onSpeedSliderInputEnded)

-- Cũng cho phép kéo trên thanh trượt chính
speedSlider.InputBegan:Connect(onSpeedSliderInput)
speedSlider.InputChanged:Connect(onSpeedSliderInputChanged)
speedSlider.InputEnded:Connect(onSpeedSliderInputEnded)
...
print("✅ Teleport & Avoid & Speed Control Script Đã Sẵn Sàng!")